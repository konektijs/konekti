...
- 스트리밍 GraphQL HTTP 응답(기본 SSE subscription 전달 포함)은 가능할 때 `FrameworkResponse.raw` 대신 어댑터가 제공하는 `FrameworkResponse.stream`을 사용합니다.
...
- 스트리밍 GraphQL/SSE 동등성을 유지하려면 커스텀 어댑터가 `FrameworkResponse.stream`을 노출해야 합니다.
- `bootstrapNodeApplication()` 같은 Node 전용 부트스트랩 헬퍼는 이제 `@konekti/runtime` 루트 배럴이 아니라 `@konekti/runtime/node`에 위치합니다.
