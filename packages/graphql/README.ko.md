...
interface ResolverMethodOptions {
  ...
  outputType?: GraphqlRootOutputType; // scalar/object 또는 listOf(scalar/object)
}
```

- `input`: 인자 매핑 및 검증에 사용할 DTO 클래스입니다.
- `argTypes`: 인자별 타입 추론 결과를 덮어씁니다. 스칼라 리터럴(예: `'string'`) 또는 리스트 입력을 위한 `listOf('string')`를 사용할 수 있습니다.
- `outputType`: resolver 반환 타입을 명시합니다(기본값: `string`). 스칼라 리터럴, named `GraphQLObjectType`, 그리고 루트 operation payload용 `listOf(...)` 래퍼를 지원합니다.
