...
- [빠른 시작](#빠른-시작)
- [일반적인 패턴](#일반적인-패턴)
  - [수동 provider 구성과 `createNotificationsProviders`](#수동-provider-구성과-createnotificationsproviders)
...
### 수동 provider 구성과 `createNotificationsProviders`

`createNotificationsProviders(...)`는 애플리케이션이 `NotificationsModule.forRoot(...)` 밖에서 동일한 provider 정규화 구성을 재사용해야 할 때 지원되는 manual-composition helper입니다.
...
- 이 helper는 `NotificationsModule.forRoot(...)`가 구성하는 `NOTIFICATIONS`, `NOTIFICATION_CHANNELS`, `NotificationsService` wiring을 동일하게 유지합니다.
- `createNotificationsProviders(...)`는 `NotificationsModule.forRoot(...)`와 같은 옵션 정규화를 적용하며, 여기에는 frozen channel 배열, events publisher가 구성되었을 때의 기본 라이프사이클 발행 활성화, 그리고 최소 대량 큐 threshold `1` 보장이 포함됩니다.
- 이 helper는 패키지 계약을 축소하지 않습니다. 직접 dispatch는 기본 경로로 유지되고, 선택적인 queue/event seam은 계속해서 애플리케이션 경계에서 명시적으로 선택해야 합니다.
...
- `packages/notifications/src/module.test.ts`: 모듈 등록, `createNotificationsProviders(...)` helper parity, async wiring, queue seam, tolerant bulk dispatch 예제.
