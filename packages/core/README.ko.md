...
애플리케이션 코드는 공개 데코레이터와 `ensureMetadataSymbol()`을 `@fluojs/core`에서 import해야 합니다. `@fluojs/core/internal` 서브패스는 `getModuleMetadata()`, `getRouteMetadata()`, `getClassDiMetadata()`, `getOwnClassDiMetadata()`, `getInheritedClassDiMetadata()` 같은 메타데이터 reader가 필요하거나, 명시적 저장소와 `Symbol.metadata`를 병합하거나, 프레임워크 수준 데코레이터를 만드는 fluo 패키지를 위한 경로입니다.

module, route, class DI 메타데이터를 노출하는 internal reader는 frozen snapshot을 반환하므로, 호출자는 반환 객체를 read-only로 취급해야 하며 직접 mutate하면 안 됩니다(`Object.isFrozen(result) === true`). `getModuleMetadata()`, `getClassDiMetadata()`, `getOwnClassDiMetadata()`는 metadata가 바뀌지 않은 상태에서 반복해서 읽을 때 stable snapshot reference를 재사용할 수 있습니다. `getInheritedClassDiMetadata()`는 frozen이고 값이 올바른 merged snapshot을 반환하지만 stable reference 재사용 계약은 제공하지 않습니다. `getRouteMetadata()`도 stored metadata와 standard decorator metadata를 매번 병합하면서 frozen route snapshot을 반환하며, 중첩된 headers와 redirect metadata도 frozen snapshot입니다. middleware, guard, interceptor 컬렉션에 저장된 runtime instance reference는 freeze 대상이 아니므로 정상적인 `handle()`, `canActivate()`, `intercept()` 상태 업데이트는 계속 동작합니다. 이 reference를 감싸는 metadata collection과 descriptor object만 freeze됩니다. **Migration note:** 기존에 `getModuleMetadata()`, `getRouteMetadata()`, `getClassDiMetadata()`, `getOwnClassDiMetadata()`, `getInheritedClassDiMetadata()`에서 반환된 metadata를 직접 mutate하던 코드는 먼저 복사본을 만들거나 지원되는 metadata writer 경로로 업데이트하도록 수정해야 합니다.
