...
Application code should import public decorators and `ensureMetadataSymbol()` from `@fluojs/core`. The `@fluojs/core/internal` subpath is reserved for fluo packages that need metadata readers such as `getModuleMetadata()`, `getRouteMetadata()`, `getClassDiMetadata()`, `getOwnClassDiMetadata()`, and `getInheritedClassDiMetadata()`, merge explicit stores with `Symbol.metadata`, or build framework-level decorators.

Internal metadata readers that expose module, route, and class DI metadata return frozen snapshots, so callers must treat returned objects as read-only (`Object.isFrozen(result) === true`) and must not mutate them directly. `getModuleMetadata()`, `getClassDiMetadata()`, and `getOwnClassDiMetadata()` can reuse stable snapshot references on repeated reads when metadata has not changed; `getInheritedClassDiMetadata()` returns frozen, value-correct merged snapshots without a stable-reference reuse contract. `getRouteMetadata()` also returns frozen route snapshots, including frozen nested headers and redirect metadata, while merging stored and standard decorator metadata for each read. Runtime instance references stored in middleware, guard, or interceptor collections are not frozen, so normal `handle()`, `canActivate()`, or `intercept()` state updates keep working; only the metadata collection and descriptor objects around those references are frozen. **Migration note:** code that previously mutated returned metadata from `getModuleMetadata()`, `getRouteMetadata()`, `getClassDiMetadata()`, `getOwnClassDiMetadata()`, or `getInheritedClassDiMetadata()` must copy it first or move the update to the supported metadata writer path.
