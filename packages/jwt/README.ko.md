...
- 리프레시 토큰(Refresh-token) 발행, 로테이션, 폐기/로그아웃 흐름은 이 패키지의 범위를 벗어나며 애플리케이션에서 직접 관리해야 합니다.
...
6. `src/verifier.test.ts` — 성공 경로, 만료된 토큰, 유효하지 않은 서명
7. `src/signer.test.ts` — 서명/검증 왕복 테스트
