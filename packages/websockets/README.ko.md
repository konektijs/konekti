...
기본 Node.js 기반 websocket 런타임을 `@fluojs/websockets`에서 사용할 때 지원되는 루트 등록 surface는 `WebSocketModule.forRoot(...)`입니다. 저수준 provider 조합 helper는 내부 구현 세부사항이며, 문서화된 루트 공개 API에 포함되지 않습니다.
...
- 루트 배럴은 의도적으로 module-first surface를 유지합니다. 패키지 수준 등록은 `WebSocketModule.forRoot(...)`를 사용하고, provider 조합 helper는 지원되는 루트 export가 아닌 내부 구현으로 취급해야 합니다.

## 런타임별 서브패스

기본 루트 Node.js alias 대신 런타임을 명시적으로 고정하고 싶다면 런타임별 서브패스를 사용하세요. 각 서브패스는 해당 `*WebSocketModule.forRoot(...)` 진입점과 일치하는 런타임 lifecycle service export를 지원합니다. 저수준 `create*WebSocketProviders(...)` helper는 내부 wiring 세부사항이며 지원되는 공개 계약에 포함되지 않습니다.
