...
이제 `PrismaService<PrismaClient>`는 `PrismaClient['$transaction']`에서 생성된 트랜잭션 클라이언트 타입을 직접 추론하므로, 별도의 `PrismaTransactionClient` 타입 주석 없이도 `current()`에서 실제 Prisma 모델 자동완성과 타입 검사를 그대로 유지합니다.
...
public package는 `PRISMA_OPTIONS`, `PrismaTransactionClient<TClient>`, `PrismaModuleOptions`, `PrismaHandleProvider`도 export합니다.
...
`PrismaTransactionClient<TClient>`는 구체적인 Prisma client 타입에서 트랜잭션 범위 클라이언트 타입을 추출해 주는 helper alias입니다. 해당 타입이 직접 필요할 때만 사용하면 됩니다.
