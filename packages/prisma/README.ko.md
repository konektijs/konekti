...
# peer dependency — 직접 생성한 클라이언트를 사용하세요
...
| 메서드 | 시그니처 | 설명 |
|---|---|---|
| `current()` | `() => TClient` | 활성 트랜잭션 클라이언트(ALS에서)를 반환하거나, 트랜잭션이 없으면 루트 클라이언트를 반환 |
| `transaction()` | `(fn: () => Promise<T>) => Promise<T>` | Prisma 인터랙티브 트랜잭션 안에서 `fn`을 실행하고, tx 클라이언트를 ALS에 저장 |
| `requestTransaction()` | `(fn: () => Promise<T>) => Promise<T>` | `transaction()`과 동일하나, 인터셉터가 요청 경계에서 사용하도록 설계됨 |
...
| 패키지 | 관계 |
|---|---|
| `@konekti/core` | `PrismaService`가 구현하는 라이프사이클 훅(`OnModuleInit`, `OnApplicationShutdown`) |
