...
Konekti 애플리케이션용 데코레이터 기반 WebSocket 게이트웨이 작성 코어이며, 런타임별 raw websocket 바인딩은 `@konekti/websocket/node`, `@konekti/websocket/bun`, `@konekti/websocket/deno`, `@konekti/websocket/cloudflare-workers` 같은 명시적 서브패스로 분리됩니다.
...
데코레이터와 공용 게이트웨이 계약은 `@konekti/websocket`에서 가져오고, 선택한 HTTP 어댑터에 맞는 런타임 전용 바인딩 서브패스를 추가하세요(`@konekti/websocket/node`, `@konekti/websocket/bun`, `@konekti/websocket/deno`, 또는 `@konekti/websocket/cloudflare-workers`).
...
이 fetch-style 런타임들은 런타임별 websocket 바인딩을 위해 공용 `{ kind: 'fetch-style', contract: 'raw-websocket-expansion', mode: 'request-upgrade', support, version: 1, reason }` seam을 사용합니다. 이 capability만으로 `@konekti/websocket/node` 지원이 생기는 것은 아닙니다.

## Bun 바인딩 서브패스

- `@konekti/websocket/bun`의 `BunWebSocketModule.forRoot()` - `@konekti/platform-bun`용 Bun 네이티브 raw websocket 바인딩 등록
- `@konekti/websocket/bun`의 `createBunWebSocketProviders()` - 커스텀 Bun websocket 모듈 구성을 위한 raw provider 반환

`@konekti/websocket/bun`은 Bun의 `{ kind: 'fetch-style', contract: 'raw-websocket-expansion', mode: 'request-upgrade', support: 'supported', version: 1, reason }` realtime capability를 소비하고 `Bun.serve()` + `server.upgrade()`로 게이트웨이를 호스팅합니다.

## Deno 바인딩 서브패스

- `@konekti/websocket/deno`의 `DenoWebSocketModule.forRoot()` - `@konekti/platform-deno`용 Deno 네이티브 raw websocket 바인딩 등록
- `@konekti/websocket/deno`의 `createDenoWebSocketProviders()` - 커스텀 Deno websocket 모듈 구성을 위한 raw provider 반환

`@konekti/websocket/deno`는 Deno의 `{ kind: 'fetch-style', contract: 'raw-websocket-expansion', mode: 'request-upgrade', support: 'supported', version: 1, reason }` realtime capability를 소비하고 `Deno.upgradeWebSocket(request)`로 게이트웨이를 호스팅합니다.

## Cloudflare Workers 바인딩 서브패스

- `@konekti/websocket/cloudflare-workers`의 `CloudflareWorkersWebSocketModule.forRoot()` - `@konekti/platform-cloudflare-workers`용 Worker 네이티브 raw websocket 바인딩 등록
- `@konekti/websocket/cloudflare-workers`의 `createCloudflareWorkersWebSocketProviders()` - 커스텀 Worker websocket 모듈 구성을 위한 raw provider 반환

`@konekti/websocket/cloudflare-workers`는 Cloudflare Workers의 `{ kind: 'fetch-style', contract: 'raw-websocket-expansion', mode: 'request-upgrade', support: 'supported', version: 1, reason }` realtime capability를 소비하고 `WebSocketPair` 기반 isolate-local/stateless 범위에서 게이트웨이를 호스팅합니다.
