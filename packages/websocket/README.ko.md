...
Konekti 애플리케이션용 데코레이터 기반 WebSocket 게이트웨이 작성 코어이며, 런타임별 raw websocket 바인딩은 `@konekti/websocket/node`, `@konekti/websocket/bun` 같은 명시적 서브패스로 분리됩니다.
...
데코레이터와 공용 게이트웨이 계약은 `@konekti/websocket`에서 가져오고, 선택한 HTTP 어댑터에 맞는 런타임 전용 바인딩 서브패스를 추가하세요(`@konekti/websocket/node` 또는 `@konekti/websocket/bun`).
...
- 그 seam을 통해 호환 가능한 Node upgrade-listener host를 노출하지 않는 fetch-style 런타임은 `@konekti/websocket/node`에서 계속 unsupported 상태이며, Bun raw websocket 호스팅은 `@konekti/websocket/bun`을 통해서만 주장합니다. 다른 런타임은 여전히 전용 바인딩이 필요합니다.
