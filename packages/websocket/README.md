...
Decorator-based WebSocket gateway authoring core for Konekti applications, with runtime-specific raw websocket bindings isolated on explicit subpaths such as `@konekti/websocket/node`, `@konekti/websocket/bun`, `@konekti/websocket/deno`, and `@konekti/websocket/cloudflare-workers`.
...
Import decorators and shared gateway contracts from `@konekti/websocket`, then add the runtime-specific binding subpath that matches your HTTP adapter (`@konekti/websocket/node` for Node upgrade-listener hosts, `@konekti/websocket/bun` for `@konekti/platform-bun`, `@konekti/websocket/deno` for `@konekti/platform-deno`, or `@konekti/websocket/cloudflare-workers` for `@konekti/platform-cloudflare-workers`).
...
- `CloudflareWorkersWebSocketModule.forRoot()` from `@konekti/websocket/cloudflare-workers` - registers the Worker-native raw websocket binding for `@konekti/platform-cloudflare-workers`
- `createCloudflareWorkersWebSocketProviders()` from `@konekti/websocket/cloudflare-workers` - returns raw providers for custom Worker websocket module composition

`@konekti/websocket/cloudflare-workers` consumes Cloudflare Workers' shared `{ kind: 'fetch-style', contract: 'raw-websocket-expansion', mode: 'request-upgrade', support: 'supported', version: 1, reason }` realtime capability and hosts gateways through `WebSocketPair` in an isolate-local/stateless scope.
...
- Fetch-style runtimes that do not expose a compatible Node upgrade-listener host through that seam remain unsupported for `@konekti/websocket/node`; Bun, Deno, and Cloudflare Workers raw websocket hosting are claimed only through their dedicated subpaths. Cloudflare Workers support remains intentionally isolate-local/stateless and does not imply Durable Object or cross-isolate coordination.
