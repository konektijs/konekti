...
내장 HTTP 캐시 키는 항상 현재 요청이 실제로 받은 concrete `request.path`를 기준으로 시작합니다. 따라서 라우트 메타데이터가 `/users/:id`로 선언되어 있어도 `/users/1`과 `/users/2`는 같은 캐시 엔트리로 합쳐지지 않습니다.

기본 `route` 전략은 이 concrete path 위에서만 쿼리 매개변수를 무시합니다. 검색 조건 등에 따라 다른 응답을 캐싱하려면 `httpKeyStrategy: 'route+query'`를 활성화하세요. 이 전략도 path param 값은 그대로 보존한 뒤 정규화된 쿼리 문자열을 덧붙입니다.
...
`httpKeyStrategy: 'full'` 역시 `route+query`와 동일하게 concrete request path를 기준으로 하며, 정규화된 쿼리 문자열을 추가합니다.
