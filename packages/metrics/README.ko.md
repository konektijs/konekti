...
Konekti 애플리케이션을 위한 Prometheus 메트릭 노출 모듈입니다. `MetricsModule`을 마운트하여 Node.js 기본 메트릭과 선택적인 저지수(low-cardinality) HTTP 요청 모니터링 기능이 포함된 `/metrics` 엔드포인트를 제공합니다.
...
- 메트릭 데이터를 Konekti의 런타임 헬스체크 및 준비 상태와 동기화하고 싶을 때.
...
import { MetricsModule, MetricsService } from '@konekti/metrics';
...
이 모듈은 플랫폼 셸 및 등록된 컴포넌트의 내부 상태를 반영하는 Konekti 전용 Gauge를 자동으로 생성합니다.

- `konekti_component_ready`: 준비 완료 시 1, 아닐 시 0.
- `konekti_component_health`: 정상 상태 시 1, 아닐 시 0.
...
- `@konekti/http`: 컨트롤러 및 미들웨어 인프라를 제공합니다.
- `@konekti/runtime`: 플랫폼 셸 및 모듈 정의 로직을 제공합니다.
- `@konekti/terminus`: 헬스체크 통합을 위해 메트릭과 함께 자주 사용됩니다.
