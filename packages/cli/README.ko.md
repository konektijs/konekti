...
이 안정적인 스타터 shape에는 런타임 facade 위의 adapter-first Fastify startup(`createFastifyAdapter({ port })` + `await app.listen()`)을 사용하는 `src/main.ts`, 런타임 모듈 엔트리포인트를 canonical `*.forRoot(...)` 네이밍(예: `ConfigModule.forRoot(...)`)으로 유지하는 `AppModule` import, 런타임 소유 `/health` + `/ready`, 스타터 소유 `/health-info/`, 그리고 공식 스타터 테스트 템플릿(`src/health/*.test.ts`, `src/app.test.ts`, `src/app.e2e.test.ts`)이 포함됩니다.
...
safe rewrite 이후에는 release-facing docs/examples를 portability redesign에 맞춰 정렬하세요. 새 HTTP 앱은 adapter-first startup(`createFastifyAdapter(...)`, `createExpressAdapter(...)`)을 우선 사용하고, Node 전용 호환 경로는 `@konekti/runtime/node`로 안내해야 합니다.
