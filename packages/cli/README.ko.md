...
이 안정적인 스타터 shape에는 런타임 facade 위의 adapter-first Fastify startup(`createFastifyAdapter({ port })` + `await app.listen()`)을 사용하는 `src/main.ts`, 런타임 모듈 엔트리포인트를 canonical `*.forRoot(...)` 네이밍(예: `ConfigModule.forRoot(...)`)으로 유지하는 `AppModule` import, 런타임 소유 `/health` + `/ready`, 스타터 소유 `/health-info/`, 그리고 공식 스타터 테스트 템플릿(`src/health/*.test.ts`, `src/app.test.ts`, `src/app.e2e.test.ts`)이 포함됩니다. 이 스캐폴드는 기본 Node.js + Fastify 경로를 유지하지만, 공개 문서의 공식 런타임 매트릭스는 출판된 `@konekti/platform-*` 패키지를 통해 Node.js, Bun, Deno, Cloudflare Workers까지 확장됩니다.
...
스캐폴드 이후 런타임별 시작 가이드가 필요하면 `@konekti/platform-bun`, `@konekti/platform-deno`, `@konekti/platform-cloudflare-workers` 공식 README를 바로 참고하세요.
