...
- **`konekti inspect <module-path>`** — 모듈 그래프 기반 런타임 진단(JSON/Mermaid) 또는 opt-in 부트스트랩 타이밍을 출력
...
이 안정적인 스타터 shape에는 `src/main.ts`의 `const app = await KonektiFactory.create(AppModule, {}); await app.listen();`, 런타임 소유 `/health` + `/ready`, 스타터 소유 `/health-info/`, 그리고 공식 스타터 테스트 템플릿(`src/health/*.test.ts`, `src/app.test.ts`, `src/app.e2e.test.ts`)이 포함됩니다.
...
`inspect`는 런타임 소비자와 동일한 `@konekti/runtime` 진단 표면(`createRuntimeDiagnosticsGraph`, `renderRuntimeDiagnosticsMermaid`)을 사용합니다. `--json`과 `--mermaid`는 컴파일된 모듈 그래프를 검사하고, `--timing`은 애플리케이션 컨텍스트를 실제로 부트스트랩한 뒤 버전 고정 타이밍 payload를 출력합니다. 루트 모듈 export 이름이 `AppModule`이 아니면 `--export <symbol>`을 사용하세요.
