...
- Transport: \`src/main.ts\` uses adapter-first Fastify startup via \`createFastifyAdapter(...)\`; use \`@konekti/runtime/node\` only for Node compatibility helpers
- Runtime matrix: the generated starter keeps the default Node.js + Fastify path, but official runtime support also includes Bun, Deno, and Cloudflare Workers via the corresponding \`@konekti/platform-*\` packages
