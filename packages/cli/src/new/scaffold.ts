...
- CORS: defaults to allowOrigin '*'; pass a \`cors\` option to \`KonektiFactory.create(..., { cors })\` to restrict origins
...
- Naming policy: runtime module entrypoints use \`*.forRoot(...)\`; helper/builders stay \`create*\` (for example \`createHealthModule()\`, \`createTestingModule(...)\`)
