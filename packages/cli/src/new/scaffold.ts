...
- Transport: \`src/main.ts\` uses adapter-first Fastify startup via \`createFastifyAdapter(...)\`; raw Node HTTP now uses \`@fluojs/platform-nodejs\`, and \`@fluojs/runtime/node\` stays reserved for compatibility helpers
- Runtime matrix: the generated starter keeps the default Node.js + Fastify path, but official runtime support also includes raw Node (\`@fluojs/platform-nodejs\`), Bun, Deno, and Cloudflare Workers via the corresponding adapter packages
...
- CORS: defaults to allowOrigin '*'; pass a \`cors\` option to \`FluoFactory.create(..., { cors, adapter: createFastifyAdapter(...) })\` to restrict origins
...
function createProjectReadme(options: BootstrapOptions, bootstrapPlan: ResolvedBootstrapPlan): string {
  return bootstrapPlan.emitter.type === 'microservice'
    ? createMicroserviceProjectReadme(options)
    : createHttpProjectReadme(options);
}
...
// Official runtime support also includes raw Node via @fluojs/platform-nodejs,
