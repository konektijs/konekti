...
const PACKAGE_DIRECTORY_BY_NAME = {
  ...
  '@konekti/di': 'di',
  '@konekti/http': 'http',
  '@konekti/platform-fastify': 'platform-fastify',
  ...
  return JSON.stringify(
    {
      ...
      dependencies: {
        ...
        '@konekti/platform-fastify': createDependencySpec('@konekti/platform-fastify', releaseVersion, packageSpecs),
...
- Transport: \`src/main.ts\` uses adapter-first Fastify startup via \`createFastifyAdapter(...)\`; raw Node HTTP now uses \`@konekti/platform-nodejs\`, and \`@konekti/runtime/node\` stays reserved for compatibility helpers
- Runtime matrix: the generated starter keeps the default Node.js + Fastify path, but official runtime support also includes raw Node (\`@konekti/platform-nodejs\`), Bun, Deno, and Cloudflare Workers via the corresponding adapter packages
...
- CORS: defaults to allowOrigin '*'; pass a \`cors\` option to \`KonektiFactory.create(..., { cors, adapter: createFastifyAdapter(...) })\` to restrict origins
...
function createMainFile(): string {
  return `import { createFastifyAdapter } from '@konekti/platform-fastify';
import { KonektiFactory } from '@konekti/runtime';

import { AppModule } from './app';

// The generated starter keeps the default Node.js + Fastify path.
// Official runtime support also includes raw Node via @konekti/platform-nodejs,
// plus Bun, Deno, and Cloudflare Workers through their dedicated adapter packages.
