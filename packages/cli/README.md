...
`fluo dev --dry-run` also reports the watch boundary. Generated Node, Bun, Deno, and Cloudflare Workers projects show `Watch mode: fluo-restart` by default, while Node `--raw-watch` and `FLUO_DEV_RAW_WATCH=1` show `Watch mode: native-watch` for debugging.

Use reporter flags when you need to tune the CLI process boundary rather than runtime app logging:

```bash
# Default: app logs only (stdout/stderr), no fluo lifecycle UI
fluo dev
