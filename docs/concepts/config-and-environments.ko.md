...
- **`@konekti/config`**: 부트스트랩에서 명시적으로 전달한 입력을 바탕으로 설정 로딩, 우선순위, 유효성 검사, 타입이 지정된 접근을 처리합니다.
- **부트스트랩 (Bootstrap)**: 환경 변수를 암묵적으로 다시 읽는 대신 호출자가 전달한 설정 입력을 소비합니다.
...
- **명시적 환경 소스**: 시스템 env를 포함하려면 애플리케이션 경계에서 `processEnv: process.env`를 명시적으로 전달합니다.
...
1.  **기본값 (Default Values)**: 하드코딩된 폴백(fallback) 값입니다.
2.  **Env 파일**: `envFile`/`envFilePath`로 지정한 경로에서 로드됩니다 (기본값 `.env`).
3.  **프로세스 환경 (Process Environment)**: 호출자가 `processEnv` 객체를 명시적으로 전달한 경우에만 포함됩니다.
4.  **런타임 재정의 (Runtime Overrides)**: 부트스트랩 도중 직접 전달됩니다.
...
일반적인 애플리케이션 설정에는 `ConfigService`를 사용하세요. env를 파라미터로 바꾸는 책임은 애플리케이션 경계에 두고, 통합 패키지는 env를 직접 읽지 말고 타입이 지정된 설정 프로바이더를 소비하도록 유지하는 것이 좋습니다.
