...
- **`@fluojs/redis` (분산 저장소)**: 여러 애플리케이션 인스턴스에 걸쳐 캐시를 유지할 수 있게 해주는 선택적 패키지입니다. `CacheModule`은 기본 Redis 경로를 그대로 사용할 수도 있고, 캐시 트래픽을 전용 연결로 분리해야 할 때 `redis.clientName`으로 이름 있는 등록을 가리킬 수도 있습니다.
