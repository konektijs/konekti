...
### 2. 프로바이더 생성
프로바이더는 비즈니스 로직이나 데이터 액세스를 처리합니다. fluo에서는 클래스에 `@Inject` 데코레이터를 사용하여 의존성을 명시적으로 선언합니다.
...
- **표준 준수**: 이 모든 흐름은 미래의 ECMAScript 표준과 일치하는 네이티브 TypeScript 데코레이터를 사용합니다.

### 다음 단계
- **반복 작업 자동화**: `fluo g module catalog` 명령어를 사용하여 이 구조를 생성해 보세요. [제너레이터 워크플로우](./generator-workflow.ko.md)에서 자세히 알아볼 수 있습니다.
