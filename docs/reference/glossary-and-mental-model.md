...
This glossary defines the core terminology and mental models that govern the Konekti framework. Use this as a lookup for technical terms and to understand the "Konekti way" of building backend applications.
...
| **Platform Adapter** | A package bridging the abstract Konekti runtime to specific environments (Node, Bun, Deno). | This abstraction allows your code to remain portable across different runtimes. |
...
| **Bootstrap Path** | The sequence from `KonektiFactory.create()` to the application being ready. | Understanding this helps in debugging startup issues and wiring lifecycle hooks. |
