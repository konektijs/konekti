...
- **`@konekti/platform-cloudflare-workers`**: 공유 `@konekti/runtime/web` fetch-style 어댑터 seam 위에 구축된 Cloudflare Workers HTTP 어댑터이며, eager/lazy Worker fetch 엔트리포인트, Worker isolate용 명시적 stateless lifecycle semantics, 그리고 contract-only raw websocket 확장 seam을 제공합니다. Cloudflare Workers의 `WebSocketPair`(주로 Durable Objects와 함께 사용) websocket 모델은 `@konekti/websocket/node`가 요구하는 Node `upgrade` 리스너 모델과 호환되지 않으므로, 전용 `@konekti/websocket/cloudflare-workers` 바인딩이 추가되기 전까지 raw websocket 지원은 contract-only로 유지됩니다.
- **`@konekti/platform-fastify`**: Fastify 기반 HTTP 어댑터.
- **`@konekti/platform-express`**: Express 기반 HTTP 어댑터.
- **`@konekti/platform-bun`**: 공용 `@konekti/runtime/web` fetch-style 어댑터 seam을 재사용해 fetch-style 런타임 패리티를 제공하고, contract-only raw websocket 확장 seam을 노출하는 HTTP 어댑터. Bun의 `Bun.serve()` + `server.upgrade()` websocket 모델은 `@konekti/websocket/node`가 요구하는 Node `upgrade` 리스너 모델과 호환되지 않으므로, 전용 `@konekti/websocket/bun` 바인딩이 추가되기 전까지 raw websocket 지원은 contract-only로 유지됩니다.
- **`@konekti/platform-deno`**: 공유 `@konekti/runtime/web` fetch-style 어댑터 seam과 contract-only raw websocket 확장 seam 위에 구축된 Deno `Deno.serve(...)` 어댑터. Deno의 `Deno.upgradeWebSocket()` websocket 모델은 `@konekti/websocket/node`가 요구하는 Node `upgrade` 리스너 모델과 호환되지 않으므로, 전용 `@konekti/websocket/deno` 바인딩이 추가되기 전까지 raw websocket 지원은 contract-only로 유지됩니다.
- **`@konekti/platform-socket.io`**: 공용 Konekti 런타임과 websocket 데코레이터 위에 구축된 Socket.IO v4 게이트웨이 어댑터이며, raw Node 서버 직접 소유를 가정하지 않고 플랫폼이 선택한 realtime capability를 소비합니다. 현재 정직한 지원 주장은 문서화·테스트된 server-backed 어댑터인 `@konekti/platform-nodejs`, `@konekti/platform-fastify`, `@konekti/platform-express`로 제한됩니다.
...
- **`@konekti/websocket`**: transport-neutral WebSocket 게이트웨이 작성용 데코레이터, 메타데이터, 디스크립터, 공용 계약을 제공합니다. 현재 raw `ws` 바인딩은 명시적 `@konekti/websocket/node` 서브패스에 있으며, 플랫폼이 선택한 server-backed realtime capability를 소비하고, 현재는 `@konekti/platform-nodejs`, `@konekti/platform-fastify`, `@konekti/platform-express`에서만 문서화·테스트된 지원 범위를 가집니다. fetch-style 어댑터는 contract-only raw websocket 확장 seam을 노출할 수 있지만, 별도 구현과 테스트가 추가되기 전까지 그것이 런타임 지원을 뜻하지는 않습니다.
