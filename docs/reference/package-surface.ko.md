...
- **`@konekti/platform-cloudflare-workers`**: 공유 `@konekti/runtime/web` fetch-style 어댑터 seam 위에 구축된 Cloudflare Workers HTTP 어댑터이며, eager/lazy Worker fetch 엔트리포인트, Worker isolate용 명시적 stateless lifecycle semantics, 그리고 이후 Worker 전용 작업을 위한 contract-only raw websocket 확장 capability를 제공합니다.
