...
- **`@konekti/platform-cloudflare-workers`**: 공유 `@konekti/runtime/web` fetch-style 어댑터 seam 위에 구축된 Cloudflare Workers HTTP 어댑터이며, eager/lazy Worker fetch 엔트리포인트, Worker isolate용 명시적 stateless lifecycle semantics, 그리고 Worker 네이티브 request-upgrade 호스팅을 위한 supported raw websocket 확장 seam을 제공합니다. 공식 raw websocket 호스팅은 전용 `@konekti/websocket/cloudflare-workers` 바인딩이 제공하며, Durable Object 분산 없이 isolate-local/stateless 범위에 한정됩니다.
- **`@konekti/platform-fastify`**: Fastify 기반 HTTP 어댑터.
- **`@konekti/platform-express`**: Express 기반 HTTP 어댑터.
- **`@konekti/platform-bun`**: 공용 `@konekti/runtime/web` fetch-style 어댑터 seam을 재사용해 fetch-style 런타임 패리티를 제공하고, Bun 네이티브 request-upgrade 호스팅을 위한 supported raw websocket 확장 seam을 노출하는 HTTP 어댑터입니다. 공식 raw websocket 호스팅은 전용 `@konekti/websocket/bun` 바인딩이 제공하며, `@konekti/websocket/node`는 계속 Node upgrade-listener 전용으로 유지됩니다.
- **`@konekti/platform-deno`**: 공유 `@konekti/runtime/web` fetch-style 어댑터 seam과 Deno 네이티브 request-upgrade 호스팅을 위한 supported raw websocket 확장 seam 위에 구축된 Deno `Deno.serve(...)` 어댑터입니다. 공식 raw websocket 호스팅은 전용 `@konekti/websocket/deno` 바인딩이 제공하며, `@konekti/websocket/node`는 계속 Node upgrade-listener 전용으로 유지됩니다.
...
- **`@konekti/websocket`**: transport-neutral WebSocket 게이트웨이 작성용 데코레이터, 메타데이터, 디스크립터, 공용 계약을 제공합니다. 런타임별 raw websocket 바인딩은 명시적 서브패스로 분리되며, `@konekti/websocket/node`는 server-backed Node upgrade-listener 호스트용, `@konekti/websocket/bun`은 `@konekti/platform-bun` 기반 Bun 네이티브 request-upgrade 호스팅용, `@konekti/websocket/deno`는 `@konekti/platform-deno` 기반 Deno 네이티브 request-upgrade 호스팅용, `@konekti/websocket/cloudflare-workers`는 `@konekti/platform-cloudflare-workers` 기반 Worker 네이티브 request-upgrade 호스팅용이며 isolate-local/stateless 범위에 머뭅니다.
