...
- **`@konekti/platform-socket.io`**: 공용 Konekti 런타임과 websocket 데코레이터 위에 구축된 Socket.IO v4 게이트웨이 어댑터이며, raw Node 서버 직접 소유를 가정하지 않고 플랫폼이 선택한 realtime capability를 소비합니다.
