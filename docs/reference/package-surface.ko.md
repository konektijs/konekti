...
- **`@konekti/platform-cloudflare-workers`**: 공유 `@konekti/runtime/web` fetch-style 어댑터 seam 위에 구축된 Cloudflare Workers HTTP 어댑터이며, eager/lazy Worker fetch 엔트리포인트, Worker isolate용 명시적 stateless lifecycle semantics, 그리고 Worker 네이티브 request-upgrade 호스팅을 위한 supported raw websocket 확장 seam을 제공합니다. 공식 raw websocket 호스팅은 전용 `@konekti/websockets/cloudflare-workers` 바인딩이 제공하며, Durable Object 분산 없이 isolate-local/stateless 범위에 한정됩니다.
- **`@konekti/platform-fastify`**: Fastify 기반 HTTP 어댑터.
- **`@konekti/platform-express`**: Express 기반 HTTP 어댑터.
- **`@konekti/platform-bun`**: 공용 `@konekti/runtime/web` fetch-style 어댑터 seam을 재사용해 fetch-style 런타임 패리티를 제공하고, Bun 네이티브 request-upgrade 호스팅을 위한 supported raw websocket 확장 seam을 노출하는 HTTP 어댑터입니다. 공식 raw websocket 호스팅은 전용 `@konekti/websockets/bun` 바인딩이 제공하며, `@konekti/websockets/node`는 계속 Node upgrade-listener 전용으로 유지됩니다.
- **`@konekti/platform-deno`**: 공유 `@konekti/runtime/web` fetch-style 어댑터 seam과 Deno 네이티브 request-upgrade 호스팅을 위한 supported raw websocket 확장 seam 위에 구축된 Deno `Deno.serve(...)` 어댑터입니다. 공식 raw websocket 호스팅은 전용 `@konekti/websockets/deno` 바인딩이 제공하며, `@konekti/websockets/node`는 계속 Node upgrade-listener 전용으로 유지됩니다.
- **`@konekti/socket.io`**: 공용 Konekti 런타임과 websocket 데코레이터 위에 구축된 Socket.IO v4 게이트웨이 어댑터이며, raw Node 서버 직접 소유를 가정하지 않고 플랫폼이 선택한 realtime capability를 소비합니다. 현재 정직한 지원 주장은 문서화·테스트된 server-backed 어댑터인 `@konekti/platform-nodejs`, `@konekti/platform-fastify`, `@konekti/platform-express`로 제한됩니다.
...
- **`@konekti/websockets`**: transport-neutral WebSocket 게이트웨이 작성용 데코레이터, 메타데이터, 디스크립터, 공용 계약을 제공합니다. 루트 데코레이터 표면은 이제 명시적인 server-backed 전용 `serverBacked: { port }` opt-in도 포함하며, 이는 현재 문서화·테스트된 server-backed 어댑터(`@konekti/platform-nodejs`, `@konekti/platform-fastify`, `@konekti/platform-express`)에서 `@konekti/websockets/node`만 소비합니다. 런타임별 raw websocket 바인딩은 명시적 서브패스로 분리되며, `@konekti/websockets/node`는 server-backed Node upgrade-listener 호스트용, `@konekti/websockets/bun`은 `@konekti/platform-bun` 기반 Bun 네이티브 request-upgrade 호스팅용, `@konekti/websockets/deno`는 `@konekti/platform-deno` 기반 Deno 네이티브 request-upgrade 호스팅용, `@konekti/websockets/cloudflare-workers`는 `@konekti/platform-cloudflare-workers` 기반 Worker 네이티브 request-upgrade 호스팅용이며 isolate-local/stateless 범위에 머뭅니다. fetch-style 런타임 바인딩은 이 server-backed opt-in을 명시적으로 거부합니다.
...
- **`@konekti/testing`**: 모듈/app 테스트 baseline 패키지입니다. 루트 배럴은 `createTestingModule(...)`, `createTestApp(...)`, 모듈 메타데이터 유틸리티에 집중하고, 목/HTTP 헬퍼/portability harness/conformance harness(여기에는 fetch-style raw websocket 확장 하니스도 포함됨)는 전용 subpath로 공개합니다.
