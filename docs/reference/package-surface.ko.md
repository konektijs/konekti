...
- **`@konekti/runtime`**: 애플리케이션 부트스트랩/런타임 오케스트레이션, 런타임 강제 플랫폼 셸 등록(`platform.components`)과 의존성 순서 start/stop, 공유 플랫폼 계약 spine 타입(`PlatformOptionsBase`, `PlatformComponent`, 라이프사이클/준비상태/헬스/진단/스냅샷 계약), 버전 고정 모듈 진단 내보내기, opt-in 부트스트랩 타이밍. 루트 배럴은 transport-neutral 경계를 유지합니다. raw Node 어댑터 선택은 이제 `@konekti/platform-nodejs`, Node 전용 호환 헬퍼는 `@konekti/runtime/node`, fetch-style request/response 브리징은 `@konekti/runtime/web`, `@konekti/runtime/internal`은 프레임워크 내부 wiring 토큰 전용으로 남기고 어댑터 헬퍼는 명시적인 internal 서브패스로 이동합니다.
- **`@konekti/platform-nodejs`**: raw Node.js HTTP 어댑터를 위한 공개 패키지이며, bare Node를 adapter-first 시작 경로로 노출하면서도 현재 런타임 소유 호환 헬퍼는 패키지 경계 뒤에 그대로 유지합니다.
- **`@konekti/platform-cloudflare-workers`**: 공유 `@konekti/runtime/web` Request/Response 브리지 위에 구축된 Cloudflare Workers HTTP 어댑터이며, eager/lazy Worker fetch 엔트리포인트와 Worker isolate용 명시적 stateless lifecycle semantics를 제공합니다.
- **`@konekti/platform-fastify`**: Fastify 기반 HTTP 어댑터.
- **`@konekti/platform-express`**: Express 기반 HTTP 어댑터.
- **`@konekti/platform-bun`**: 공용 `@konekti/runtime/web` Request/Response 브리지를 재사용해 fetch-style 런타임 패리티를 제공하는 Bun 기반 HTTP 어댑터.
- **`@konekti/platform-deno`**: 공유 런타임 Web request/response 코어 위에 구축된 Deno `Deno.serve(...)` 어댑터.
