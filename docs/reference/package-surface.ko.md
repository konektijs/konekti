...
- **`@konekti/websocket`**: transport-neutral WebSocket 게이트웨이 작성용 데코레이터, 메타데이터, 디스크립터, 공용 계약을 제공합니다. 현재 raw `ws` 바인딩은 명시적 `@konekti/websocket/node` 서브패스에 있으며, 플랫폼이 선택한 server-backed realtime capability를 소비하고, 현재는 `@konekti/platform-nodejs`, `@konekti/platform-fastify`, `@konekti/platform-express`에서만 문서화·테스트된 지원 범위를 가집니다.
