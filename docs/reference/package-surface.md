...
- **`@konekti/platform-cloudflare-workers`**: Cloudflare Workers HTTP adapter built on the shared `@konekti/runtime/web` fetch-style adapter seam, including eager/lazy Worker fetch entrypoints, explicit stateless lifecycle semantics for Worker isolates, and the shared contract-only raw websocket expansion seam. Cloudflare Workers' `WebSocketPair` (often paired with Durable Objects) websocket model is incompatible with the Node `upgrade`-listener model required by `@konekti/websocket/node`, so raw websocket support remains contract-only until a dedicated `@konekti/websocket/cloudflare-workers` binding lands.
- **`@konekti/platform-fastify`**: Fastify-based HTTP adapter.
- **`@konekti/platform-express`**: Express-based HTTP adapter.
- **`@konekti/platform-bun`**: Bun-based HTTP adapter that reuses the shared `@konekti/runtime/web` fetch-style adapter seam for fetch-style runtime parity and exposes the shared contract-only raw websocket expansion seam. Bun's `Bun.serve()` + `server.upgrade()` websocket model is incompatible with the Node `upgrade`-listener model required by `@konekti/websocket/node`, so raw websocket support remains contract-only until a dedicated `@konekti/websocket/bun` binding lands.
- **`@konekti/platform-deno`**: Deno `Deno.serve(...)` adapter built on the shared `@konekti/runtime/web` fetch-style adapter seam and the shared contract-only raw websocket expansion seam. Deno's `Deno.upgradeWebSocket()` websocket model is incompatible with the Node `upgrade`-listener model required by `@konekti/websocket/node`, so raw websocket support remains contract-only until a dedicated `@konekti/websocket/deno` binding lands.
...
- **`@konekti/websocket`**: Transport-neutral WebSocket gateway authoring decorators, metadata, descriptors, and shared contracts. The current raw `ws` binding lives on the explicit `@konekti/websocket/node` subpath, consumes the platform-selected server-backed realtime capability, and is currently documented/tested for `@konekti/platform-nodejs`, `@konekti/platform-fastify`, and `@konekti/platform-express` only. Fetch-style adapters may expose the shared contract-only raw websocket expansion seam, but that does not count as runtime support until a dedicated implementation and tests land.
...
- **`@konekti/testing`**: Testing baseline for module/app flows. The root barrel stays focused on `createTestingModule(...)`, `createTestApp(...)`, and module introspection, while mocks, HTTP helpers, portability harnesses, and conformance harnesses — including the fetch-style raw websocket expansion harness — are published on dedicated subpaths.
