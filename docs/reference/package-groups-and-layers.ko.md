...
| `graphql` | HTTP `/graphql` 엔드포인트 + code-first/schema-first 리졸버 | core, di, dto-validator, http, runtime |
...
| Protocol Adapters (websocket, graphql, metrics) | ② Request Pipeline + ⑥ Protocol Adapters | graphql 재분류, metrics 제거, platform-socket.io 추가 |
