...
| **Microservice starter** | **Scaffolded now** | Node.js + no HTTP platform + TCP via `--shape microservice --transport tcp --runtime node --platform none`, Redis Streams via `--transport redis-streams`, MQTT via `--transport mqtt`, and gRPC via `--transport grpc` | TCP remains the simplest default starter baseline when you omit `--transport`; Redis Streams, MQTT, and gRPC now ship as runnable starters with transport-specific dependency/env/proto wiring. Redis Pub/Sub, NATS, Kafka, and RabbitMQ remain documented validation-only families for now. |
