...
| **CLI self-update prompt** | `fluo <command>` in an interactive TTY | Checks the cached npm `latest` dist-tag for `@fluojs/cli`; when newer, asks before running `pnpm add -g @fluojs/cli@<latest>` and reruns the same argv under the updated `fluo` binary. `fluo new` and `fluo create` bypass the fresh cache before scaffolding so first-run starter creation observes newly published CLI behavior sooner; other commands keep the normal cache TTL. CI, non-TTY, npm-script, `FLUO_NO_UPDATE_CHECK=1`, `--no-update-check`, and registry failures skip the prompt. |
...
| **HTTP starter testing layout** | Generated non-Deno HTTP application starters | Generates fast unit tests under `src/greeting/`, a slice/module graph test at `src/greeting/greeting.slice.test.ts`, app dispatch tests in `src/app.test.ts`, and the dedicated e2e-style suite at `test/app.e2e.test.ts` using `createTestApp({ rootModule })`. Generated Vitest config includes both `src/**/*.test.ts` and `test/**/*.test.ts`, while supported scripts include `test`, `test:watch`, `test:cov`, and `test:e2e`. Existing `src/app.e2e.test.ts` users can move those tests to `test/app.e2e.test.ts` without changing the request helper. |
...
| **Resource Generation** | `fluo g <type>` | Produces consistent naming suffixes (`.service.ts`, `.controller.ts`). Request DTOs may target an explicit feature directory with `fluo g req users CreateUser`. `fluo g module users --with-test` emits `src/users/user.slice.test.ts`, `fluo g resource users --with-slice-test` emits resource-level provider override coverage, and `fluo g e2e users` emits `test/users.e2e.test.ts` with `createTestApp({ rootModule })`. |
...
| **Slice test** | `.slice.test.ts` | `users/user.slice.test.ts` from module/resource test generation |
