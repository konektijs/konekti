...
| **Project Creation (microservice)** | `fluo new my-service --shape microservice --transport tcp --runtime node --platform none` | Generates the runnable single-package TCP microservice starter. `--transport redis-streams`, `--transport mqtt`, and `--transport grpc` now scaffold runnable starter variants with transport-specific dependency/env/proto wiring, while transport validation still recognizes the broader documented microservice families: `tcp`, `redis`, `redis-streams`, `nats`, `kafka`, `rabbitmq`, `mqtt`, `grpc`. |
