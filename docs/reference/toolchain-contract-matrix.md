...
| **Project Creation (microservice)** | `fluo new my-service --shape microservice --transport tcp --runtime node --platform none` | Generates the runnable single-package TCP microservice starter. Transport validation also recognizes the documented microservice families: `tcp`, `redis`, `redis-streams`, `nats`, `kafka`, `rabbitmq`, `mqtt`, `grpc`. |
