...
| **Development watch restart** | `fluo dev` in a Node.js generated project | Uses the fluo-owned Node restart runner by default. The runner debounces filesystem bursts, hashes content before restart, and ignores noisy output/cache/editor paths. This is restart-on-watch, not true HMR. `fluo dev --raw-watch` or `FLUO_DEV_RAW_WATCH=1` restores the runtime-native `node --watch` command for debugging. Bun, Deno, and Cloudflare Workers continue to use runtime-native dev watchers. |
