...
| **CLI self-update prompt** | `fluo <command>` in an interactive TTY | Checks the cached npm `latest` dist-tag for `@fluojs/cli`; when newer, asks before running `pnpm add -g @fluojs/cli@<latest>` and reruns the same argv under the updated `fluo` binary. CI, non-TTY, npm-script, `FLUO_NO_UPDATE_CHECK=1`, `--no-update-check`, and registry failures skip the prompt. |
