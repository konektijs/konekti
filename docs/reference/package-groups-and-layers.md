...
| `throttler` | 데코레이터 기반 요청 레이트 리미팅 | core, di, http, runtime |
| `cache-manager` | HTTP 응답 캐싱 인터셉터 + 범용 캐시 서비스 | core, di, http, runtime |
...
| `runtime` | 모듈 그래프 컴파일 + DI/HTTP/config 조립 → 앱 셸 | config, core, di, http |
| `platform-fastify` | Fastify 기반 HTTP 어댑터 (Node 기본 어댑터 대비 ~2x 성능) | http, runtime |
| `terminus` | 의존성 인식 헬스체크 (`/health`, `/ready` 강화) | core, di, http, runtime |
...
| 패키지 | 설명 | 의존 (@konekti/*) |
|---|---|---|
| `websocket` | `ws` 기반 WebSocket 게이트웨이 (공유 HTTP 서버 업그레이드) | core, di, http, runtime |
...
1. **`core`는 모놀리식 파운데이션이다** — 29개 중 27개 패키지가 `core`에 의존한다. `core`가 흔들리면 전체가 흔들린다.
...
4. **redis는 인프라 레이어의 게이트키퍼다** — `queue`, `cron`, `throttler`의 Redis 옵션은 모두 `redis` 패키지를 통해 들어간다. `redis`는 `runtime`에 의존하므로 Layer 2에 위치한다.
