...
| **CLI self-update prompt** | interactive TTY에서 `fluo <command>` 실행 | 캐시된 npm `latest` dist-tag로 `@fluojs/cli` 버전을 확인합니다. 새 버전이 있으면 `pnpm add -g @fluojs/cli@<latest>` 실행 여부를 먼저 묻고, 승인된 경우 같은 argv로 업데이트된 `fluo` 바이너리를 다시 실행합니다. CI, non-TTY, npm-script, `FLUO_NO_UPDATE_CHECK=1`, `--no-update-check`, registry 실패에서는 prompt를 건너뜁니다. |
