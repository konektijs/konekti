...
| **개발 watch restart** | Node.js 생성 프로젝트에서 `fluo dev` 실행 | 기본적으로 fluo가 소유한 Node restart runner를 사용합니다. 이 runner는 filesystem burst를 debounce하고, restart 전에 content hash를 비교하며, noisy output/cache/editor 경로를 무시합니다. 이 동작은 true HMR이 아니라 restart-on-watch입니다. 디버깅이 필요하면 `fluo dev --raw-watch` 또는 `FLUO_DEV_RAW_WATCH=1`로 런타임 native `node --watch` 명령을 복원할 수 있습니다. Bun, Deno, Cloudflare Workers는 계속 런타임 native dev watcher를 사용합니다. |
