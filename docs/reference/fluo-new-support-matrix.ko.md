...
| **마이크로서비스 스타터** | **지금 스캐폴딩됨** | `--shape microservice --transport tcp --runtime node --platform none`으로 생성되는 Node.js + 비HTTP 플랫폼 + TCP, `--transport redis-streams`로 생성되는 Redis Streams, `--transport mqtt`로 생성되는 MQTT, `--transport grpc`로 생성되는 gRPC | `--transport`를 생략하면 TCP가 가장 단순한 기본 스타터 기준선으로 유지됩니다. Redis Streams, MQTT, gRPC는 이제 transport별 dependency/env/proto 구성을 갖춘 runnable starter로 제공되며, Redis Pub/Sub, NATS, Kafka, RabbitMQ는 당분간 문서화된 validation-only 계열로 남습니다. |
