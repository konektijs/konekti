...
| Exported services | `JwtModule`은 `DefaultJwtSigner`, `DefaultJwtVerifier`, `JwtService`를 등록합니다. `RefreshTokenService`는 `refreshToken` 옵션이 구성된 경우에만 등록됩니다. | `packages/jwt/src/module.ts` |
...
| Issuer and audience | `issuer` 또는 `audience`가 구성된 경우 verifier는 `iss` 또는 `aud`가 일치하지 않는 토큰을 거부해야 합니다. | `packages/jwt/src/signing/verifier.ts` |
