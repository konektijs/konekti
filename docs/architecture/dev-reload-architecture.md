...
| Configuration file changes with config reload enabled | `createConfigReloader(...)` can watch the configured env file when `watch: true`, `ConfigModule.forRoot({ watch: true, ... })` activates that watcher during `onApplicationBootstrap()`, and `ConfigReloadModule` activates the injectable reload-layer watcher during the same lifecycle phase. The watcher skips reload when final env file content matches the last committed watch baseline. | The `ConfigService` snapshot is replaced in process after content changes and validation succeeds. | `packages/config/src/load.ts`, `packages/config/src/module.ts`, `packages/config/src/reload-module.ts` |
