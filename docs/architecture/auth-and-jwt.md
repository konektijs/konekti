...
| Exported services | `JwtModule` registers `DefaultJwtSigner`, `DefaultJwtVerifier`, and `JwtService`. `RefreshTokenService` is registered only when `refreshToken` options are configured. | `packages/jwt/src/module.ts` |
...
| Issuer and audience | When `issuer` or `audience` is configured, the verifier MUST reject tokens whose `iss` or `aud` claims do not match the configured values. | `packages/jwt/src/signing/verifier.ts` |
