...
This contract defines the technical requirements and architectural conventions for authoring third-party extensions, platform adapters, and community integration packages for the fluo framework. Adherence to these standards ensures cross-runtime compatibility and prevents metadata collisions.

## When this document matters

- **Extension Development**: When building a reusable library that integrates with fluo's DI or metadata system.
- **Platform Adapters**: When porting fluo to a new HTTP runtime (e.g., Lambda, Cloudflare Workers, or a custom internal server).
- **Integration Packages**: When wrapping existing libraries (e.g., Stripe, Auth0, or custom SQL drivers) for use within fluo modules.

---

## Metadata and Decorators

fluo uses TC39 standard decorators and a `Symbol`-based metadata system. To prevent collisions between the framework and third-party extensions, follow these strict naming rules.

### Metadata Key Naming
Custom metadata keys MUST use a namespaced `Symbol.for()` pattern to ensure uniqueness across the ecosystem.
- **Format**: `Symbol.for('fluo.extension.[package-name].[category]')`
- **Example**: `Symbol.for('fluo.extension.audit-logger.policy')`

### Authoring Decorators
Use the `metadata` property on the decorator context. Always use the `@fluojs/core` compatibility boundary (`metadataSymbol`) to ensure the symbol exists in the current environment.

```ts
import { metadataSymbol } from '@fluojs/core';

const AUDIT_KEY = Symbol.for('fluo.extension.audit-logger.policy');

export function Audit(policy: string) {
  return (value: Function, context: ClassDecoratorContext) => {
    // metadataSymbol is guaranteed by @fluojs/core
...
Platform adapters bridge the fluo HTTP runtime to specific transport implementations.

### `HttpApplicationAdapter` Interface
Every adapter must implement the core interface from `@fluojs/http`:
...
Adapters are responsible for mapping native objects to fluo's `FrameworkRequest` and `FrameworkResponse` abstractions.
...
| :--- | :--- | :--- |
| **Tier 1** | Stable | `@fluojs/core` (Decorators, Types), `HttpApplicationAdapter`. |
...
| **Tier 4** | Experimental | `@fluojs/runtime` assembly logic, Compiler internals. |
