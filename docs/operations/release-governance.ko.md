...
추가 런타임이나 통합은 package README나 거버넌스 문서에 명시적으로 문서화되지 않는 한 공개 보장이 아닙니다. 현재 공식 런타임 지원에는 Node.js, Bun, Deno가 포함됩니다. Cloudflare Workers와 기타 fetch 스타일 어댑터는 출판된 어댑터 계약과 함께 패키지 문서/테스트가 승격되기 전까지 **preview** 상태를 유지합니다.
