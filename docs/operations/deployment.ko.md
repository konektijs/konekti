...
`@konekti/runtime/node`의 `runNodeApplication()`은 `SIGTERM`과 `SIGINT` 신호를 감지하여 애플리케이션의 종료 시퀀스를 자동으로 실행합니다.
...
- 이전에 `@konekti/runtime`에서 `runNodeApplication()`을 import 했다면 `@konekti/runtime/node`로 변경하세요.
