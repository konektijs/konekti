...
이 문서는 Konekti 프레임워크의 프로덕션 배포 표준, 컨테이너화 패턴 및 런타임 상태 지표를 정의합니다. 이를 통해 애플리케이션이 다양한 클라우드 환경에서 회복 탄력성, 관측 가능성 및 관리 용이성을 유지할 수 있도록 보장합니다.

## 이 문서가 필요한 경우

- **프로덕션 준비**: Konekti 애플리케이션을 라이브 환경에 배포하기 위해 준비할 때.
...
Konekti는 이미지 크기를 최소화하고 공격 표면을 줄이기 위해 멀티 스테이지(Multi-stage) Docker 빌드를 권장합니다.
...
Konekti는 자동화된 상태 모니터링을 위한 내장 엔드포인트를 제공합니다. 이는 Kubernetes의 `livenessProbe` 및 `readinessProbe`와 직접 통합됩니다.
...
Konekti 런타임(특히 `@konekti/runtime/node`의 `runNodeApplication`)은 `SIGTERM` 및 `SIGINT` 신호를 자동으로 수신하여 깨끗한 종료 절차를 시작합니다.
