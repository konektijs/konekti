...
The release workflow is triggered automatically on pushes to `main`. When pending changesets exist, the Changesets action opens a "Version Packages" PR that bumps versions, updates changelogs, and removes consumed changesets. Merging that PR triggers the publish step, which publishes affected packages to npm with token-backed npm authentication and provenance, then creates scoped git tags and GitHub Releases. The publish step passes the same `secrets.NPM_TOKEN` value as both `NPM_TOKEN` for the Changesets action and `NODE_AUTH_TOKEN` for the npm user config path generated by `actions/setup-node`.
