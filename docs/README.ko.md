...
Konekti 문서 허브에 오신 것을 환영합니다. 간단한 REST API부터 복잡한 마이크로서비스 아키텍처까지, 이 가이드를 통해 현대적인 TypeScript의 강력한 기능을 최대한 활용할 수 있습니다.

## 🏁 시작하기

Konekti가 처음이라면, 몇 분 안에 애플리케이션을 실행할 수 있는 다음 단계부터 시작하세요.
...
- **[부트스트랩 및 시작 절차](./getting-started/bootstrap-paths.ko.md)**: Konekti가 애플리케이션 수명 주기와 초기화를 처리하는 방식.
- **[용어 사전 및 멘탈 모델](./reference/glossary-and-mental-model.ko.md)**: 프레임워크의 핵심 개념과 언어.

## 🧠 핵심 개념

Konekti의 설계 철학과 그 이면의 "왜"와 "어떻게"를 이해합니다.
...
알림 제품군(`@konekti/notifications`, `@konekti/email`, `@konekti/email/node`, `@konekti/slack`, `@konekti/discord`)은 위의 패키지 선택 가이드와 package surface 기준 문서를 통해 탐색할 수 있습니다.
...
- **[차이점 안내 (Parity Gaps)](./operations/nestjs-parity-gaps.ko.md)**: Konekti가 NestJS와 다른 점과 그 이유에 대한 정직한 문서.

---
### 다른 자료를 찾고 계신가요?
- **[실행 가능한 예제](../examples/README.ko.md)**: 실제 코드를 통해 Konekti가 작동하는 모습을 확인하세요.
