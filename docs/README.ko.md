...
[Release Governance](./operations/release-governance.ko.md)는 공개 패키지의 기준 intended publish surface, release-readiness gate, 내부 `workspace:^` dependency-range 정책, 또는 CI 전용 단건 패키지 릴리스 운영 절차와 companion gate인 `pnpm verify:platform-consistency-governance`, supervised-auto 릴리스 경계를 확인할 때 기준 문서로 사용하세요.
