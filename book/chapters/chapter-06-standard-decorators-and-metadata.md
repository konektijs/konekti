...
이 장은 Konekti의 심장에 가까운 내용을 다룬다. Konekti에서 `core`는 단순한 유틸리티 모음이 아니라, **프레임워크가 이해할 수 있는 선언을 코드에 새기는 층**이다.

## 이 장이 중요한 이유

독자는 보통 DI나 HTTP가 더 흥미롭다고 느낀다. 하지만 실제로는 이 장을 제대로 이해하지 못하면 이후 장은 전부 “프레임워크가 뭔가 알아서 해 주는 것”처럼 보이기 쉽다. Konekti의 많은 기능은 `core`에서 시작된 메타데이터 위에 서 있기 때문이다 `[pkg:core/README.md]`.

## Konekti에서 데코레이터는 무엇을 하는가

Konekti의 데코레이터는 오래된 TypeScript 방식의 “숨겨진 메타데이터 생성 장치”가 아니다. Konekti는 TC39 표준 데코레이터를 사용하고, 필요한 메타데이터를 프레임워크가 직접 관리하는 registry에 기록한다 `[repo:docs/concepts/decorators-and-metadata.md]`.
...
flowchart LR
  ...
  B --> C[Metadata Helper]
  C --> D[Konekti Metadata Store]
  D --> E[DI reads inject/scope]
  D --> F[Runtime reads module metadata]
  D --> G[HTTP reads route/DTO metadata]
```

이 도표는 Konekti의 decorator layer가 왜 “즉시 실행”이 아니라 “기록 후 소비” 구조인지 한눈에 보여 준다. 중요한 점은 decorator가 곧바로 인스턴스를 만들거나 라우트를 열지 않고, **metadata store를 중심으로 sibling package들이 나중에 읽는 구조**를 만든다는 것이다 `[repo:docs/concepts/decorators-and-metadata.md]` `[pkg:core/src/decorators.ts]` `[pkg:core/src/metadata/class-di.ts]`.
...
이 파일은 전체가 61줄이다. 네 개의 데코레이터(`Module`, `Global`, `Inject`, `Scope`)가 전부이고, 모두 같은 패턴을 따른다. metadata helper를 호출하고, 그것으로 끝이다. `Module`도, `Inject`도, 직접적으로 module graph를 만들지 않고, 인스턴스를 생성하지도 않는다. 이것이 Konekti의 중요한 미학이다. **decorator는 기록만 하고, 해석은 나중 계층으로 미룬다.**

## 메타데이터는 어디에 모이는가

`packages/core/src/metadata.ts`는 여러 종류의 메타데이터 reader/writer를 다시 내보내는 진입점 역할을 한다 `[pkg:core/src/metadata.ts]`. 여기서 중요한 점은 Konekti가 메타데이터를 흩어진 방식으로 다루지 않는다는 것이다.
...
이 코드는 Konekti의 메타데이터 철학을 아주 압축적으로 보여 준다. 첫째, metadata write는 덮어쓰기만 하지 않고 기존 값을 보존하는 merge 성격을 가진다. `read()`로 기존 값을 꺼내고 `write()`로 합친 결과를 저장하는 2단계 패턴이 핵심이다. 둘째, metadata read는 단순 현재 클래스 조회가 아니라 **상속을 반영한 effective metadata 조회**다 `[pkg:core/src/metadata/class-di.ts]`.
...
이 설계는 중요하다. Konekti는 metadata를 한 번 적고 끝내는 단순 키-값 저장소가 아니라, **상속과 split decorator pass를 고려한 구조**로 다룬다.

## `shared.ts`는 왜 메인테이너에게 중요할까

`packages/core/src/metadata/shared.ts`는 표면적으로는 utility처럼 보이지만, 실제로는 Konekti metadata 시스템의 공용 기반이다 `[pkg:core/src/metadata/shared.ts]`.
...
이 짧은 코드가 말해 주는 것은 분명하다. Konekti는 표준 메타데이터 접근점인 `Symbol.metadata`를 프레임워크 차원에서 명시적으로 보장한다 `[pkg:core/src/metadata/shared.ts#L20-L34]`. 즉, 메타데이터 접근 경로 자체가 우연한 구현 세부사항으로 남아 있지 않다.
...
이 심볼 집합은 Konekti 내부에서 메타데이터가 단순 문자열 키가 아니라 **명시적 namespace를 가진 계약 집합**임을 보여 준다 `[pkg:core/src/metadata/shared.ts#L75-L84]`. 메인테이너 관점에서는 바로 이 지점이 중요하다. 패키지가 늘어나도 어떤 metadata가 어떤 용도인지 namespace 차원에서 안정적으로 구분되기 때문이다.
...
이 함수들이 말해 주는 것은 단순하다. Konekti는 metadata를 “있으면 읽는 값”으로 두지 않고, **방어적으로 복제하고 merge 규칙을 명시한 상태 데이터**로 다룬다.
...
이 구분을 잡는 순간 Konekti 구조가 훨씬 맑아진다.
...
즉, 메타데이터는 Konekti 내부에서 **공통 언어** 역할을 한다.

## 이 장의 핵심 문장

> Konekti의 데코레이터는 일을 직접 처리하는 도구가 아니라, **다른 패키지들이 공통으로 읽을 수 있는 계약을 기록하는 도구**다.

## 심화 워크스루 1: 왜 `decorators.ts`가 이렇게 짧아야 하는가

초심자는 종종 이런 질문을 한다. “왜 `@Inject` 안에서 바로 뭔가 더 많은 일을 하지 않지?” 하지만 바로 그 점이 중요하다. `decorators.ts`가 짧다는 것은 Konekti가 decorator layer를 **부작용이 적은 선언 계층**으로 유지하려 한다는 뜻이다 `[pkg:core/src/decorators.ts]`.
...
즉, Konekti는 metadata를 “있으면 읽고 없으면 넘어가는 느슨한 데이터”로 다루지 않는다. 오히려 **프레임워크 전 패키지가 공유하는 준-불변 계약 데이터**처럼 다룬다.
...
이 두 함수가 짧다는 사실은 오히려 중요하다. `Global()`은 module visibility라는 매우 큰 효과를 만들지만, 그 효과 자체는 runtime/module-graph 단계에서 읽혀야 한다. `Scope()`도 마찬가지로 provider lifecycle에 큰 영향을 주지만, 실제 해석은 DI container가 맡는다. 즉, **효과가 크다고 해서 decorator가 더 많은 일을 해야 하는 것은 아니다**. Konekti는 끝까지 “decorator는 기록만 한다”는 규칙을 지킨다.
...
- `metadataKeys`는 Konekti가 직접 소유하고 관리하는 저장소 키 집합이다.

이 구분은 중요하다. 프레임워크가 표준 데코레이터 세계와 연결되면서도, 내부 contract를 느슨하게 흩뿌리지 않고 자기 namespace 안에 보존하기 때문이다. 즉, Konekti는 **표준과 내부 계약을 동시에 관리하는 이중 채널**을 갖는다.
...
앞에서 `class-di.ts`와 `shared.ts`를 봤다면, 이제 `metadata/store.ts`도 함께 봐야 한다 `[pkg:core/src/metadata/store.ts]`. 이 파일은 작지만 Konekti metadata 계층의 핵심 철학을 아주 노골적으로 드러낸다.
...
이 구현은 매우 중요하다 `[pkg:core/src/metadata/store.ts]`. Konekti는 metadata store를 단순 WeakMap으로 쓰지 않고, **read와 write 양쪽에서 clone을 강제하는 계약**으로 감싼다. 즉, 값을 읽는 쪽도 원본을 훼손할 수 없고, 값을 쓰는 쪽도 외부 참조를 그대로 저장하지 못한다.

이 철학은 앞에서 본 `shared.ts`와 정확히 맞물린다. Konekti는 metadata를 “가변 객체를 여기저기 넘기며 공유하는 상태”가 아니라, clone-on-read/write를 거친 **방어적 계약 데이터**로 취급한다.
...
즉, core 장은 decorator 문법만 설명해서는 안 된다. 실제로는 **기록 대상, 저장 매체, clone 규칙, 상속 해석**까지 합쳐져야 비로소 Konekti metadata 시스템이 보인다.
...
Konekti 책이 여기서 차별화되어야 하는 이유는 분명하다. Konekti의 강점은 표면 API가 아니라, **그 표면 API가 어떤 metadata contract 위에 서 있는지까지 추적 가능하다는 점**이기 때문이다.
...
이 질문이 가능해지는 순간, Konekti는 데코레이터 프레임워크가 아니라 **metadata를 중심으로 조립된 계약형 시스템**으로 보이기 시작한다.

## 6장의 마지막 문장

이 장을 끝내는 가장 좋은 문장은 아마 이럴 것이다.

> Konekti의 핵심은 데코레이터 문법 그 자체가 아니라, **그 데코레이터가 남긴 기록을 다른 계층이 일관되게 읽을 수 있게 만드는 metadata contract**다.
