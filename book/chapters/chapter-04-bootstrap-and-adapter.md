...
이 장에서는 fluo 앱이 실제로 어디서 시작되는지 살펴본다. JavaScript 중급자에게 가장 먼저 보여줘야 할 것은 “프레임워크가 뭔가를 알아서 시작한다”가 아니라, **부트스트랩은 명시적으로 일어난다**는 사실이다.
...
3. 둘을 조합하는 책임은 `fluoFactory`에 있다.

## 왜 어댑터를 직접 넘기는가

fluo는 플랫폼 의존성을 숨기지 않고 **어댑터 인수로 드러낸다** `[repo:docs/concepts/architecture-overview.md]`. 이는 “이 앱은 지금 어떤 런타임 위에 올라가는가?”를 부트스트랩 코드만 보고도 알 수 있게 한다.
...
여기에는 서버를 띄우는 코드가 없다. 대신 “이 앱은 어떤 구성요소로 이루어져 있는가?”만 적혀 있다. 이 차이가 중요하다. fluo에서 **구조 선언**과 **실행 시작**은 서로 다른 층의 책임이다.

## 부트스트랩을 읽는 올바른 순서

fluo 앱을 처음 읽을 때는 보통 다음 순서가 가장 좋다.
...
minimal 예제는 작은 만큼 중요하다. `hello.controller.ts`와 `hello.service.ts`를 함께 보면, fluo가 요구하는 최소 feature shape가 보인다 `[ex:minimal/src/hello.controller.ts]` `[ex:minimal/src/hello.service.ts]`.
...
이 두 파일이 중요한 이유는, fluo의 많은 고급 논의가 결국 이 정도 단순한 구조에서 시작되기 때문이다. controller는 service를 주입받고, service는 plain class로 동작한다. 즉, 복잡한 runtime/DI/HTTP 논의도 결국은 **이 최소 조립 단위가 커져 가는 과정**으로 이해할 수 있다.
...
이 코드는 readiness를 단순 boolean으로 다루지 않는다는 점을 보여 준다 `[pkg:runtime/src/platform-shell.ts#L54-L82]`. fluo는 component별 report를 모아 `ready`, `degraded`, `not-ready` 같은 더 풍부한 상태를 계산한다. 즉, bootstrap 뒤편의 운영 엔진도 꽤 정교하다.
...
즉, fluo runtime은 작은 hello-world 예제 뒤에서 이미 **운영 가능한 수명주기 엔진**을 갖고 있다.
...
fluo 부트스트랩은 짧지만 숨겨져 있지 않다. 어댑터를 직접 넘기고, 루트 모듈을 직접 지정하고, factory를 통해 앱을 만든다. 이 구조가 이후 core, di, runtime, http를 모두 연결하는 첫 번째 관문이다.
