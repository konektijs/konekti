...
이 파트는 Konekti가 무엇인지, 왜 존재하는지, 그리고 구현 세부사항에 들어가기 전에 어떤 멘탈 모델을 가져야 하는지를 설명한다.
...
Konekti는 TC39 표준 데코레이터, 명시적 의존성 주입, 멀티 런타임 이식성을 기반으로 한 TypeScript 백엔드 프레임워크로 자신을 정의한다 `[repo:README.md]`.

### 2장. Konekti 멘탈 모델

문서 허브와 용어집을 바탕으로 Konekti를 “데코레이터가 메타데이터를 적고, runtime/di/http가 그것을 읽어 실행한다”는 관점으로 설명한다 `[repo:docs/README.md]` `[repo:docs/reference/glossary-and-mental-model.md]`.
...
- 루트 README는 Konekti를 “performance without magic”, “explicit over implicit”, “run anywhere”의 조합으로 설명한다 `[repo:README.md]`.
