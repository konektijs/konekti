...
이 장은 Konekti를 “쓴다”에서 “지킨다”로 넘어가는 문턱이다. 메인테이너는 기능을 추가하는 사람일 뿐 아니라, 동작 계약을 깨뜨리지 않는 사람이어야 한다 `[repo:docs/operations/release-governance.md]`.

## 왜 이 장이 마지막이어야 하는가

테스트와 메인테이너 워크플로우는 중요하지만, 초반부에 오면 오히려 무게중심이 흐려진다. 독자가 먼저 이해해야 하는 것은 Konekti가 어떻게 동작하는가이고, 그 다음에야 “그 동작을 어떻게 지킬 것인가”가 자연스럽게 들어온다. 이 장이 후반부에 오는 이유는 바로 그 순서 때문이다.

## 테스트 계층을 왜 먼저 보아야 하나

testing guide는 검증을 type safety, unit isolation, module wiring, runtime parity 순으로 정리한다 `[repo:docs/operations/testing-guide.md]`. 이 구조는 Konekti가 테스트를 단순 coverage 숫자가 아니라 **신뢰 계층**으로 본다는 뜻이다.
...
이 다이어그램은 Konekti의 품질 문화를 한 줄로 보여 준다. 테스트는 coverage를 채우기 위한 체크박스가 아니라, unit → wiring → dispatcher → portability → CI gate로 이어지는 **누적 신뢰 구조**다 `[repo:docs/operations/testing-guide.md]` `[repo:package.json]` `[repo:.github/workflows/ci.yml]` `[pkg:testing/src/app.ts]`.
...
이 한 줄은 Konekti 메인테이너 문화의 축약판이다 `[repo:package.json#L20-L29]`. build, typecheck, lint, test가 분리된 작업이 아니라 하나의 연속된 gate라는 점이 중요하다. 즉, 메인테이너에게 “거의 다 됐다”는 상태는 없고, verify를 통과했느냐 아니냐만 남는다.

## worktree가 왜 워크플로우의 일부인가

Konekti는 worktree 기반으로 이슈 작업을 분리하는 흐름을 장려한다 `[repo:CONTRIBUTING.md]`. 이 방식은 책 집필 작업에도 잘 맞는다. 주제별/이슈별로 작업 맥락을 분리할 수 있기 때문이다.
...
이 발췌는 CI가 단일 test 잡 하나로 끝나지 않는다는 점을 보여 준다 `[repo:.github/workflows/ci.yml]`. Konekti의 verify 문화는 빌드, 타입, 린트, 테스트, portability, governance, release readiness를 서로 다른 gate로 분리해 둔다. 즉, 품질은 하나의 체크가 아니라 **여러 차원의 검증 묶음**이다.
...
이 코드가 보여 주는 것은 “서버를 띄우지 않고도 real runtime stack을 통과하는 테스트가 가능하다”는 점이다 `[pkg:testing/src/app.ts#L57-L82]`. 즉, Konekti 테스트는 모의 객체만 쓰는 세계와 실제 런타임을 전부 올리는 세계 사이에 아주 실용적인 중간 지점을 제공한다.
...
이 책의 마지막 장이 여기서 끝나는 이유는 분명하다. Konekti를 깊게 이해한 사람의 최종 모습은 “기능을 더 빨리 만드는 사람”이 아니라, **시스템이 앞으로도 계속 신뢰 가능하도록 지키는 사람**이기 때문이다.
...
CI 잡 중에서 특히 눈여겨봐야 할 것은 web runtime adapter portability 잡이다 `[repo:.github/workflows/ci.yml#L129-L158]`. 많은 프로젝트는 Node 환경에서 통과하면 만족하지만, Konekti는 Bun, Deno, Cloudflare Workers까지 별도 matrix로 검증한다.

이것이 의미하는 바는 단순하다. Konekti가 README에서 말하는 “run anywhere”는 마케팅 문구가 아니라, **실제 CI 잡으로 강제되는 약속**이라는 뜻이다 `[repo:README.md]` `[repo:.github/workflows/ci.yml]`.
...
Konekti가 메인테이너에게 요구하는 능력은 후자다. 그래서 이 장은 책의 결론으로서 충분히 무거워야 한다.
...
좀 더 강하게 말하면, 메인테이너는 기능을 추가하는 사람이라기보다 **시스템의 신뢰를 유지하는 사람**이다. 이 관점이 책의 마지막 장에서 분명해져야, 독자는 Konekti를 단순 프레임워크 사용법이 아니라 하나의 엔지니어링 문화로 받아들이게 된다.
