...
fluo에서 커스텀 데코레이터의 진정한 위력은 가드 및 인터셉터와의 통합에서 나옵니다. 메타데이터를 기록하는 데코레이터와 해당 메타데이터를 읽는 가드를 결합함으로써 성능이 뛰어나고 사용하기 쉬운 복잡한 도메인 특화 로직을 구축할 수 있습니다.

예를 들어, 어떤 메서드를 데이터베이스에 로깅해야 하는지 기록하는 `@AuditLog()` 데코레이터를 가정해 보십시오. `AUDIT_LOG_METADATA_KEY`를 확인하고 메타데이터가 있는 경우 요청 및 응답 세부 정보를 기록하는 전역 인터셉터를 만들 수 있습니다.

### 패턴: "메타데이터-가드 쌍"

이 패턴은 두 가지 구성 요소로 이루어집니다.
...
커스텀 데코레이터가 예상대로 작동하지 않을 때, 첫 번째 단계는 메타데이터가 올바르게 기록되고 있는지 확인하는 것입니다. 2장에서 논의한 것처럼 유닛 테스트 내에서 `getModuleMetadata` 또는 `getClassDiMetadata` 헬퍼를 사용하여 데코레이트된 클래스의 메타데이터 가방을 검사할 수 있습니다.

메타데이터가 존재한다면 문제는 해당 메타데이터를 읽어야 하는 구성 요소(가드, 인터셉터 또는 DI 컨테이너)에 있을 가능성이 높습니다. 메타데이터 검색 시점부터 실행을 추적하는 것이 병목 현상을 식별하는 가장 빠른 방법입니다.
...
다음 장에서는 fluo의 DI 컨테이너가 복잡한 프로바이더 그래프를 해석하기 위해 이러한 원칙들을 어떻게 사용하는지 살펴보겠습니다.
...
지금까지 fluo의 표준 우선 메타데이터 시스템을 활용하는 커스텀 데코레이터를 구축하는 방법을 보았습니다. `@CurrentUser()`와 `@Roles()`의 구현을 탐구했고, 가드 및 인터셉터 통합의 고급 패턴에 대해 논의했습니다.

이제 4장에서 fluo의 DI 컨테이너가 이러한 원칙들을 어떻게 사용하는지 살펴보겠습니다.
