...
즉 고급 패턴의 핵심은 이것입니다.
Fluo에서 잘 설계된 커스텀 데코레이터는 크고 복잡한 실행기가 아니라, 작고 조합 가능한 메타데이터 writer입니다.
진짜 런타임의 힘은 나중에 다른 패키지들이 그 레코드를 읽고 HTTP 동작, 문서화, DI 정책으로 변환할 때 나타납니다.
...
- **상속**: 기본적으로 메서드에 기록된 메타데이터는 `path:packages/core/src/metadata/class-di.ts`에서 본 것처럼 상속 로직을 명시적으로 처리하지 않는 한 하위 클래스에 상속되지 않습니다.

커스텀 데코레이터를 마스터함으로써 여러분은 사실상 fluo 내에서 여러분만의 도메인 특화 언어를 구축하고 있는 것입니다. 여러분은 코드를 더 표현력 있고 가독성 있게 만들며 유지 관리하기 쉽게 만드는 일련의 추상화를 만들고 있습니다.
...
다음 장에서는 fluo의 DI 컨테이너가 복잡한 프로바이더 그래프를 해석하기 위해 이러한 원칙들을 어떻게 사용하는지 살펴보겠습니다. 지금까지 fluo의 표준 우선 메타데이터 시스템을 활용하는 커스텀 데코레이터를 구축하는 방법을 보았습니다. `@CurrentUser()`와 `@Roles()`의 구현을 탐구했고, 가드 및 인터셉터 통합의 고급 패턴에 대해 논의했습니다. 이제 4장에서 fluo의 DI 컨테이너가 이러한 원칙들을 어떻게 사용하는지 살펴보겠습니다.
