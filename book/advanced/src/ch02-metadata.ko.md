...
조금 멀리서 보면 일관된 설계 휴리스틱이 드러납니다.

- 언어가 안정된 훅을 제공하면 표준을 사용합니다.
- 사용자 공간과 충돌하면 안 되는 이름에는 심볼을 사용합니다.
- 소유권과 가비지 컬렉션을 함께 가야 할 때는 `WeakMap`을 사용합니다.
- 읽기/쓰기 쪽이 변이 권한을 공유하면 안 될 때는 값을 복제합니다.
- 상속이 중요할 때만 지연 계보 순회를 수행하고, 모든 것을 미리 계산하지 않습니다.
- 배열과 키의 병합은 숨은 마법이 아니라 명시적 순서 규칙으로 처리합니다.

이 휴리스틱들이 Fluo를 작게 유지하면서도 복잡한 상위 패키지를 가능하게 합니다.
`@fluojs/http`, `@fluojs/openapi`, `@fluojs/validation` 같은 패키지는
별도의 리플렉션 우주를 만들 필요가 없습니다.
같은 메타데이터 프리미티브를 재사용하고,
자신들의 심볼 키와 읽기/쓰기 헬퍼만 특화하면 됩니다.
그래서 생태계가 통합되어 보이는 이유는 하나의 거대한 레지스트리가 모든 관심사를
소유해서가 아니라,
공유된 메타데이터 계약 위에서 각 패키지가 조립되기 때문입니다.

실무 관점의 결론도 분명합니다.
직접 확장 패키지를 설계할 때는 데코레이터를 쓰기 전에 세 가지를 물어야 합니다.

- 이 데이터는 상호운용성을 위해 표준 메타데이터 bag에 있어야 하는가?
- 이 데이터는 수명 주기를 프레임워크가 소유하므로 `WeakMap` 저장소에 있어야 하는가?
- 상속 시 병합 규칙이 필요한가, 아니면 own metadata만으로 충분한가?

이 질문들에 명확히 답할 수 있다면,
이미 Fluo 코어와 같은 메타데이터 모델로 사고하고 있는 것입니다.
바로 그 정렬이 커스텀 통합을 덧댄 느낌이 아니라 native extension처럼 보이게 만듭니다.
...
fluo의 메타데이터 모델은 불변성의 원칙 위에 구축되었습니다. 우리가 클래스에 대한 메타데이터를 해석할 때, 단순히 살아있는 객체에 대한 참조를 반환하는 것이 아니라, 해당 시점의 메타데이터에 대해 신중하게 구성된 뷰를 반환합니다. 이는 메타데이터가 하위 소비자에 의해 우발적으로 수정되어 발생하는 일반적인 버그들을 방지합니다.

이러한 불변성은 `path:packages/core/src/metadata/store.ts:16-33`에 있는 `createClonedWeakMapStore` 유틸리티를 사용하여 달성됩니다. 이 스토어는 모든 읽기 및 쓰기 작업에서 방어적 복제(defensive clone)가 수행되도록 보장하여, 원래 메타데이터 레코드의 무결성을 보존합니다.
...
표준 우선 접근 방식을 사용할 때의 과제 중 하나는 메타데이터가 종종 심볼과 `WeakMap` 뒤에 숨겨져 있어 디버깅 중에 검사하기가 더 어렵다는 것입니다. 이를 해결하기 위해 fluo는 특정 클래스에 대한 내부 메타데이터 상태를 노출할 수 있는 일련의 진단 도구를 제공합니다.

fluo 모노레포의 일부인 `Studio` 진단 패키지를 사용하여 모듈 그래프와 모든 프로바이더와 연결된 메타데이터를 시각화할 수 있습니다. 이는 애플리케이션이 어떻게 구성되고 있는지 이해하고 구성 문제를 조기에 식별하는 데 매우 유용한 도구입니다.

### KSR 참조: packages/core/src/metadata/store.ts

fluo가 이를 어떻게 관리하는지 진정으로 이해하려면 `createClonedWeakMapStore`의 구현을 살펴보십시오. 네이티브 `WeakMap` API를 활용하면서도 불변성과 타입 안전성을 보장하기 위해 방어적 복제 계층을 어떻게 추가했는지 보게 될 것입니다.
...
이러한 원칙들을 따름으로써 우리는 고성능일 뿐만 아니라 견고하고 유지 관리가 쉬운 메타데이터 시스템을 구축했습니다. 다음 장에서는 이 시스템을 활용하여 여러분만의 커스텀 데코레이터를 구축하는 방법을 살펴보겠습니다.
...
핵심 메타데이터 모델이 어떻게 작동하는지 확인했으므로, 이제 여러분만의 커스텀 데코레이터를 구축할 시간입니다. 3장에서 데코레이터 합성의 기초부터 시작하겠습니다.
