...
운영 단계에서는 프레임워크가 정리 작업을 한 곳에서 조정할 수 있습니다.

헬스 체크, 배포, 롤링 재시작이 들어오기 시작하면 이런 구조가 특히 도움이 됩니다.

몇 가지 설정값은 따로 주목할 필요가 있습니다.

- `host`는 어떤 네트워크 인터페이스에 노출할지 결정합니다.
- `port`는 호출자가 기대할 서비스 계약을 정의합니다.
- provider 목록은 어떤 핸들러를 발견할 수 있는지 결정합니다.
...
프레임워크가 그 일을 맡기 때문에 호출 코드가 도메인 동작에 집중할 수 있습니다.

그렇다고 이 호출이 로컬 함수처럼 바뀌는 것은 아닙니다.

여전히 네트워크 호출이며, 네트워크 고유의 문제가 그대로 따라옵니다.

따라서 호출자는 타임아웃 예산, 폴백 동작, 원격 서비스 비가용 상황을 여전히 고민해야 합니다.
...
서비스가 자주 이 한계에 가까워진다면 문제는 대개 계약 설계에 있습니다.

잘못된 채널로 바이너리 데이터를 보내고 있을 수 있습니다.

과도한 데이터를 한 번에 조회하고 있을 수 있습니다.
...
너무 짧으면 일시적인 지연 스파이크가 바로 사용자 실패로 확대됩니다.

너무 길면 더 이상 의미 없는 응답을 기다리느라 리소스를 묶어 둡니다.

재시도 역시 맥락 의존적입니다.

멱등적인 읽기에는 재시도가 안전할 수 있습니다.

상태를 바꾸는 작업에서는 멱등성 보호 없이 재시도하면 중복 실행이 발생할 수 있습니다.
...
그 시점에 버퍼된 바이트를 JSON으로 파싱하고, 적절한 핸들러로 패킷을 디스패치합니다.

장점은 분명합니다.

- 프레이밍 포맷을 직접 확인하기 쉽습니다.
...
- 원격 서비스가 의도적으로 요청을 거절한 것인가?
- 요청 완료 전에 네트워크가 실패한 것인가?
...
Kubernetes `Service`, 사이드카, 혹은 NGINX나 HAProxy 같은 고전적인 프록시가 안정적인 네트워크 진입점을 제공할 수 있습니다.
...
- **Safety Boundaries**: fluo의 1 MiB 프레임 제한은 메모리 기반 남용을 줄여 줍니다.
- **Point-to-Point**: TCP는 대상 서비스가 알려진 주소에서 도달 가능해야 합니다.
