...
RabbitMQ는 이 파트에서 처음으로 노골적으로 큐 중심적이라는 느낌을 주는 브로커입니다.

Redis Streams도 이미 FluoShop에 내구성 있는 전달을 제공했습니다.

하지만 RabbitMQ는 큐 토폴로지 자체를 주된 설계 도구로 끌어올립니다.

이 점은 한 서비스가 작업을 소유하고, 다른 서비스가 재시도하며, 세 번째 서비스는 브로커가 전달 준비가 되었다고 판단할 때까지 기다려야 하는 구조에서 중요합니다.

FluoShop v1.3.0에서는 결제 이후의 fulfillment handoff를 RabbitMQ로 옮깁니다.

Order Service는 여전히 이전 장들의 트랜스포트로 고객 트래픽을 받습니다.

새로운 RabbitMQ 경로는 결제가 성공한 뒤에 시작됩니다.

이 시점의 비즈니스는 더 이상 즉각적인 사용자 지연 시간만을 요구하지 않습니다.

피킹, 포장, 후속 알림을 위한 안정적인 작업 큐를 요구합니다.

## 4.1 Why RabbitMQ in FluoShop

RabbitMQ는 토폴로지가 로그보다 큐에 가깝게 설계될 때 특히 잘 맞습니다.

Fulfillment Service는 포장 작업을 직접 소유해야 합니다.

Notification Service는 fulfillment milestone을 들어야 하지만 주 작업자가 되어서는 안 됩니다.

운영자는 큐 깊이를 보고 지연이 실제 병목인지 일시적 현상인지 판단할 수 있어야 합니다.

이런 스타일은 일반적인 요청 경로보다 RabbitMQ에서 더 자연스럽습니다.
...
패키지를 정확히 하나의 작업자가 포장해야 하는 순간, 큐는 fan-out topic보다 더 많은 정보를 줍니다.

## 4.2 Bootstrapping RabbitMQ with caller-owned collaborators

fluo 트랜스포트는 RabbitMQ를 마법 같은 연결 관리자 뒤에 숨기지 않습니다.

패키지 README가 설명하듯 브로커 클라이언트는 caller-owned collaborator로 남습니다.

즉 애플리케이션이 채널을 만들고, 큐를 선언하고, publish 또는 consume 함수를 transport에 전달해야 합니다.

이것은 의도적인 설계입니다.

프레임워크는 메시지 라우팅을 소유합니다.

애플리케이션은 인프라 배선을 계속 소유합니다.
...
덕분에 여러 서비스 인스턴스가 동시에 살아 있을 때도 응답 충돌을 막을 수 있습니다.
...
바로 이 연속성이 intermediate 책을 반복이 아닌 누적형 학습으로 만들어 줍니다.

## 4.3 Queue topology for request and event traffic

RabbitMQ는 큐 이름을 명시적으로 설계하도록 유도합니다. 큐가 단순한 파이프가 아니기 때문입니다.
...
FluoShop에서는 command 성격의 message와 event 성격의 broadcast를 위한 큐를 분리합니다.
...
event 큐의 볼륨을 보면 broadcast 성격의 side effect가 얼마나 활발한지도 볼 수 있습니다.

### 4.3.2 Instance-scoped response queues

저장소의 RabbitMQ 테스트는 중요한 안전 속성을 검증합니다.

동시 실행 중인 인스턴스가 서로의 응답을 가로채면 안 됩니다.

그래서 기본 `responseQueue`에 `crypto.randomUUID()`가 포함됩니다.

FluoShop에서는 이 덕분에 Order Service를 수평 확장하더라도 각 인스턴스가 자신이 기다리는 fulfillment reply를 안전하게 받을 수 있습니다.

`responseQueue`를 직접 재정의한다는 것은 공유 reply topology를 직접 소유하겠다는 뜻입니다.

그 선택이 틀린 것은 아닙니다.

다만 상관관계와 수명 주기 정책도 직접 책임져야 합니다.

안전한 기본값은 인스턴스 범위 응답 큐를 그대로 두는 것입니다.
...
transport는 요청 프레임을 직렬화하고, `requestId`와 `replyTo`를 담아 전송한 뒤, 응답 프레임이 도착하면 호출자를 resolve 또는 reject 합니다.

### 4.4.1 FluoShop packer reservation

FluoShop에서 Order Service는 때때로 broker-backed quick answer를 Fulfillment로부터 받아야 합니다.

예를 들어 같은 날 출고를 약속하기 전에 창고 wave에 충분한 packer capacity가 있는지 물을 수 있습니다.
...
여기서 비즈니스 이점은 미묘하지만 분명합니다.

Order Service는 창고 내부로 향하는 직접 TCP 소켓이 필요한 것이 아닙니다.
...
반면 브로커 경로 자체가 timeout이라면 일시적 의존성 오류로 보여 주는 편이 맞습니다.
...
Payment가 `payment.settled`를 발행하면 여러 반응이 이어질 수 있습니다.

Fulfillment는 피킹을 스케줄합니다.

Notification은 고객 메시지를 준비합니다.

리스크 시스템은 체크포인트를 남길 수 있습니다.

이제 결제 경로는 모든 downstream side effect를 기다릴 필요가 없습니다.
...
여기서 바뀌지 않는 것이 무엇인지 보아야 합니다.

핸들러는 여전히 단순한 provider method입니다.

transport는 큐 프레임을 맡습니다.

domain service는 비즈니스 결정을 맡습니다.

이것이 책 전반에 반복되는 fluo 패턴입니다.
...
`pickwave.created`가 반복해서 실패한다면 운영자는 원래 order context를 보존한 채 poison message를 격리할 수 있어야 합니다.
...
- `send()`는 반드시 `listen()` 이후에 호출해야 합니다.
- timeout은 호출자를 명확하게 reject 합니다.
- 동시 요청도 `requestId`로 안전하게 상관관계가 유지됩니다.
...
토폴로지, 재시도, 큐 소유권이 책임 있게 정의될 때만 충분히 안전한 durability가 됩니다.
...
- ready message count
- unacked 또는 in-flight work
- 배포 이후 redelivery 증가 여부
- 인스턴스별 response queue churn
- dead-letter queue 증가
...
- API read는 TCP에 남을 수 있습니다.
- payment durability는 Redis Streams에 남을 수 있습니다.
- warehouse work만 RabbitMQ queue로 이동합니다.
...
대칭성을 위해 모든 트랜스포트를 한꺼번에 옮길 필요는 없습니다.
...
이 시점에서 FluoShop은 세 가지 서로 다른 통신 스타일을 갖습니다.

TCP는 단순한 직접 조회를 맡습니다.

Redis Streams는 돈이 걸린 durability를 보호합니다.

RabbitMQ는 stream replay보다 work assignment가 중요한 창고 큐를 소유합니다.

이런 transport diversity는 혼란이 아니라 강점입니다.
