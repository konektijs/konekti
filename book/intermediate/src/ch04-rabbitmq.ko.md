...
FluoShop에서는 이 덕분에 Order Service를 수평 확장하더라도 각 인스턴스가 자신이 기다리는 fulfillment reply를 안전하게 받을 수 있습니다. 이는 요청 헤더의 `replyTo` 필드가 소비자에게 정확히 어디로 결과를 보내야 할지 알려주는 **Direct Reply-to** 개념(또는 임시 큐)을 사용하여 구현됩니다.
