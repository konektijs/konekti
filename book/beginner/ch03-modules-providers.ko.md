...
애플리케이션이 커질수록 우발적 결합(accidental coupling)—시스템의 모든 부분이 서로에 대해 너무 많이 아는 상태—은 가장 큰 유지보수 문제 중 하나가 됩니다. 이 "스파게티 코드" 느낌의 복잡도는 나중에 코드를 고칠 때 예기치 못한 문제를 일으키곤 합니다. 생산성과 개발자 행복을 해치는 침묵의 살인마와 같죠.

어떤 파일이든 다른 모든 파일에 자유롭게 접근할 수 있다면 코드베이스를 이해하기가 매우 어려워지며, 단일 변경 사항이 멀리 떨어진 영역에서 예상치 못한 부작용을 일으킬 수 있습니다. 모듈은 공유를 기본값이 아니라 의식적인 선택으로 바꾸어 그 혼란을 늦춰 줍니다. 이러한 "옵트인" 공유 모델은 개발자가 내부 API와 외부 API에 대해 신중하게 생각하도록 유도합니다. 이러한 의도적인 설계가 대규모 애플리케이션을 가능하게 하고, 유지보수하기 쉽게 하며, 여러 해 동안 지속 가능하게 만듭니다.
...
대부분의 초보자 로직은 `DEFAULT` 스코프에 머무는 것이 좋습니다. 성능 면에서 가장 유리하고 추론하기도 쉽습니다. 요청 스코프 프로바이더는 매 요청마다 의존성 그래프의 일부를 다시 인스턴스화해야 하므로 성능에 영향을 줄 수 있습니다.
...
컨트롤러는 지배자가 아니라 조율자여야 합니다. 교통 경찰처럼 들어오는 요청을 적절한 서비스로 안내하고 결과를 반환하는 역할을 합니다.
...
처음 요청을 직접 받는 파일에 모든 것을 넣고 싶어지는 것은 자연스러운 반응입니다. 데이터가 들어오고 나가는 것을 볼 수 있기 때문에 코드를 작성하기에 가장 논리적인 장소처럼 느껴지기 때문입니다.

그 본능은 이해할 수 있지만, 엔드포인트 수가 늘어나고 동일한 로직을 반복하게 되면 고통으로 돌아옵니다. 처음부터 컨트롤러를 얇게 유지하면 나중의 정리 작업을 줄일 수 있으며, 애플리케이션이 성장하더라도 모듈화되고 유지보수 가능한 상태를 유지할 수 있습니다.
...
이 절제가 중요한 이유는 수동 생성이 인터셉터, 검증, 메타데이터와 같은 컨테이너 관리 동작을 우회하고, 일관된 의존성 그래프의 장점을 약화시키기 때문입니다. fluo가 인스턴스화를 처리하게 함으로써 프레임워크의 모든 기능이 예측 가능하게 유지되도록 보장합니다.
...
객체 생성이 비즈니스 메서드 안에 숨겨져 있지 않을수록 좋은 테스트를 작성하기 쉬워지며, 인프라 요구 사항이 아닌 테스트 중인 로직에 집중할 수 있습니다.
...
`exports`가 중요한 이유는 모든 내부 클래스가 자동으로 공개되지 않도록 막아 주기 때문입니다. 이 "캡슐화"는 모듈이 의도한 것만 노출하도록 보장하여 내부 세부 정보를 숨깁니다.
...
초보자는 하나의 import 문제가 생기면 모든 것을 export해 버리는 방식으로 대응하기도 합니다. 즉각적인 컴파일러 오류는 해결될 수 있지만, 피해야 할 습관입니다.
...
이 질문은 아키텍처가 과하게 새지 않도록 도와주며 모듈이 잘 정의되고 유지보수 가능하게 유지되도록 합니다.
...
하지만 이 책이 모듈을 초반에 소개하는 이유는 우발적인 파일 성장 대신 기능 경계 중심으로 구조를 잡는 습관을 먼저 만들기 위해서입니다. 처음부터 이 구조를 확립함으로써 나중에 분리하기 어려운 "거대한 진흙 덩어리"가 되는 것을 방지합니다.
