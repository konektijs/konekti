...
Prisma은 명시적이고 타입 안전한 개발이라는 Fluo의 철학에 완벽하게 부합하는 현대적인 객체 관계 매핑(ORM) 도구입니다. 복잡한 클래스 기반 데코레이터나 모호한 마법에 의존하는 기존 ORM과 달리, Prisma는 데이터베이스 구조와 TypeScript 타입의 단일 진실 공급원(Single Source of Truth) 역할을 하는 중앙 "스키마" 파일을 사용합니다.
...
`@fluojs/prisma`를 사용함으로써, 여러분은 프레임워크의 나머지 부분과 동일한 "표준 우선" 원칙을 따르는 데이터베이스 계층을 선택하게 됩니다. 컬럼이나 테이블에 대한 독자적인 데코레이터가 없으며, 모든 것이 Prisma 스키마를 통해 처리됩니다. 이는 데이터베이스 로직을 더 쉽게 마이그레이션하고 팀 간에 공유할 수 있게 하며, 데코레이터가 많은 다른 ORM 대안들보다 훨씬 더 안정적으로 만들어 줍니다.

더욱이, 이러한 분리는 TypeScript 코드를 비즈니스 로직에 집중할 수 있도록 깨끗하게 유지하면서도, 데이터베이스 통신이라는 무거운 작업은 검증된 네이티브 엔진이 처리하도록 보장합니다.
...
Prisma의 가장 강력한 기능 중 하나는 인트로스펙션입니다. 이미 운영 중인 데이터베이스가 있다면, Prisma는 해당 구조를 "읽어서" 자동으로 `schema.prisma` 파일이 생성될 수 있도록 해줍니다. 이는 이미 데이터베이스가 존재하는 기존 프로젝트에 `fluo`와 Prisma를 도입하는 것을 매우 쉽게 만듭니다. 수천 줄의 모델 코드를 수동으로 작성할 필요 없이 Prisma가 무거운 작업을 대신 수행하므로, 여러분은 테이블 매핑보다는 기능을 구축하는 데 집중할 수 있습니다.

### Type-Safe Queries by Default
Prisma를 사용하면 작성하는 모든 쿼리가 기본적으로 타입 안전합니다. 존재하지 않는 필드를 선택하려 하거나 숫자형 컬럼에 문자열을 전달하려고 하면, 코드를 실행하기도 전에 TypeScript 컴파일러가 에러를 잡아냅니다. 이러한 수준의 안전성은 `fluo` 철학과 완벽하게 일치하며, 기존 Node.js 애플리케이션을 괴롭히던 수많은 런타임 에러를 제거합니다. 데이터베이스 계층은 불안의 원천이 아닌 확신의 원천이 됩니다.
...
Prisma는 PostgreSQL, MySQL, SQLite, SQL Server, CockroachDB, 그리고 MongoDB를 포함한 광범위한 데이터베이스를 지원합니다. FluoBlog의 경우, 로컬 개발을 위해 PostgreSQL이나 SQLite를 추천합니다. SQLite는 별도의 데이터베이스 서버를 설치할 필요 없이 로컬 파일에 데이터를 저장하므로 초보자에게 특히 편리합니다. 하지만 프로덕션 환경의 애플리케이션을 위해서는 PostgreSQL과 같은 견고한 관계형 데이터베이스가 표준입니다.
...
`AppModule`에서의 전역 등록을 위해 `forRoot`를 사용하지만, Fluo는 특정 기능 세트를 위한 `PrismaModule`의 스코프별 등록도 지원합니다. 이를 통해 하나의 애플리케이션에서 여러 개의 Prisma Client를 사용할 수 있습니다. 예를 들어, 기본 트랜잭션 데이터베이스와 보조 분석용 웨어하우스에 동시에 연결해야 하는 경우 매우 유용합니다. 이러한 유연성은 fluo의 모듈형 설계가 갖는 주요 장점이며, 단순한 블로그에서 복잡한 다중 데이터베이스 시스템으로 애플리케이션을 발전시킬 수 있게 해줍니다.
...
다른 많은 프레임워크에서는 트랜잭션 내부에서 여러 작업을 수행하려면 서비스 레이어의 모든 함수 호출에 "트랜잭션 객체"를 전달해야 합니다. 이는 비즈니스 로직을 오염시키고 코드 가독성을 떨어뜨립니다. fluo의 `current()` 패턴은 의존성 주입 시스템을 사용하여 활성 클라이언트를 관리함으로써 이 문제를 해결합니다. 리포지토리는 트랜잭션 내부인지 외부인지 신경 쓸 필요 없이 그저 `current()` 클라이언트를 사용하기만 하면 되며, fluo가 작업에 적합한 클라이언트를 보장합니다.

### The current() Pattern in Real Projects
많은 프레임워크에서는 로우(raw) 데이터베이스 클라이언트를 사용할지 아니면 관리형 리포지토리를 사용할지 선택해야 합니다. fluo의 `PrismaClient.current()` 패턴은 두 방식의 장점을 모두 제공합니다. 이 패턴은 데이터 접근이 현재 실행 컨텍스트(예: 단일 HTTP 요청)로 제한되도록 보장하면서도, 클라이언트 인스턴스를 수동으로 전달해야 하는 번거로움을 없애줍니다.
...
데이터베이스 통합을 마스터함으로써 fluo를 사용하여 프로덕션 준비가 된 백엔드를 구축하는 큰 걸음을 내디뎠습니다. 데이터베이스가 구축됨에 따라 FluoBlog는 이제 게시물을 안정적으로 저장하고 검색할 수 있습니다. 하지만 실제 데이터 작업에서는 여러 단계가 함께 성공하거나 실패해야 하는 경우가 많습니다. 다음 장에서는 **트랜잭션(Transactions)**을 사용하여 이러한 시나리오를 처리하는 방법을 배우겠습니다.
...
데이터베이스 통합은 단순히 테이블에 연결하는 것 그 이상입니다. 이는 애플리케이션 전체를 위한 신뢰할 수 있고 성능이 뛰어난 토대를 구축하는 일입니다. Prisma의 강력함과 fluo의 구조 및 명시성을 결합함으로써 여러분은 장기적인 성공을 위한 준비를 하고 있는 것입니다.

데이터베이스 스키마와 마이그레이션을 핵심 소스 코드의 일부로 취급하는 것을 잊지 마세요. 리포지토리를 가볍고 집중된 상태로 유지하고, 코드가 트랜잭션을 인식하도록 항상 `current()` 패턴을 사용하세요. 이러한 원칙들을 염두에 둔다면 fluo를 이용한 백엔드 개발을 마스터하는 길에 성큼 다가서게 될 것입니다.
