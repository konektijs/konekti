...
`fluo` 애플리케이션에 요청이 들어오면 일련의 레이어를 거치게 됩니다. 가드는 중요한 지점에 위치합니다. 가드는 `ExecutionContext`에 접근할 수 있어 들어오는 요청, 대상 컨트롤러, 특정 핸들러 메서드에 대한 모든 정보를 알고 있습니다. 이 컨텍스트를 통해 가드는 'admin'은 라우트에 접근을 허용하고 'user'는 차단하거나, 요청이 특정 IP 범위에서 시작되었는지 확인하는 등 지능적인 결정을 내릴 수 있습니다.
...
FluoBlog에서 가장 흔히 쓰이는 전략은 `JwtStrategy`입니다. 이 전략은 헤더에서 토큰을 추출하고, 서명을 검증하며, 페이로드를 다시 `JwtPrincipal`로 변환합니다. "표준 우선(Standard-First)" 철학을 따름으로써, Fluo는 여러분의 JWT 구현이 RFC 7519와 같은 산업 표준과 호환되도록 보장합니다.

### 15.3.1 Defining the Strategy Class
전략 클래스는 `PassportStrategy` 기본 클래스를 상속받는 일반적인 Fluo 공급자(Provider)입니다. 이 기본 클래스는 여러분의 로직을 Passport.js 엔진에 연결하는 복잡한 작업을 처리합니다.
...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(payload: any) {
    // 이 반환값은 자동으로 req.user에 첨부됩니다.
    // fluo에서는 정규화된 Principal 객체를 반환합니다.
    return {
      subject: payload.sub,
      roles: payload.roles || [],
...
생성자의 `super()` 호출은 Passport 전략의 동작을 설정하는 곳입니다.
- `jwtFromRequest`: 토큰을 추출하는 방법을 정의합니다. `fromAuthHeaderAsBearerToken()`은 대부분의 API에서 표준이지만, 쿠키나 커스텀 쿼리 파라미터에서 추출하도록 설정할 수도 있습니다.
- `ignoreExpiration`: `true`로 설정하면 만료된 토큰도 수락합니다. 프로덕션 환경에서는 거의 항상 `false`로 유지해야 합니다.
- `secretOrKey`: 토큰의 서명을 검증하는 데 사용되는 암호화 비밀키입니다. 이는 하드코딩하지 않고 항상 `ConfigService`를 통해 로드해야 합니다.

### 15.3.3 The Validate Method: Your Security Gate
`validate()` 메서드는 모든 Passport 전략의 핵심입니다. Passport가 토큰의 서명과 만료 시간을 성공적으로 확인하면 디코딩된 페이로드와 함께 이 메서드를 호출합니다. 이때 추가적인 검사를 수행할 기회가 주어집니다. 예를 들어, 데이터베이스에서 사용자 계정이 정지되었는지 확인하거나 비밀번호가 최근에 변경되어 재로그인이 필요한지 확인하고 싶을 수 있습니다.
...
프로덕션 환경에서는 종종 `JwtStrategy`에 `UsersService`를 주입하고 데이터베이스 조회를 수행하여 사용자가 여전히 존재하고 활성 상태인지 확인합니다. 이 "검증 루프(Verification Loop)"는 민감한 금융 또는 개인 데이터를 다루는 시스템과 같이 즉각적인 권한 취소 기능이 필요한 시스템에서 매우 중요합니다. 사용자를 데이터베이스와 대조하여 검증함으로써, "무상태(Stateless)이지만 잠재적으로 오래된(stale)" 보안에서 성능과 실시간 제어를 결합한 "하이브리드" 보안으로 나아갈 수 있습니다.

### 15.3.4 Token Revocation Strategies
표준 JWT는 무상태이므로 만료되기 전에 쉽게 폐기할 수 없습니다. 하지만 `validate()` 메서드를 사용하여 폐기 패턴을 구현할 수 있습니다. 한 가지 일반적인 접근 방식은 데이터베이스에 "JWT 버전" 또는 "마지막 비밀번호 변경 타임스탬프"를 저장하고 이를 토큰의 클레임과 비교하는 것입니다. 토큰의 버전이 데이터베이스의 버전보다 오래된 경우 요청을 거부합니다. 이를 통해 모든 기기에서 즉시 로그아웃시키거나 비밀번호 재설정을 강제하는 기능을 구현할 수 있습니다.

또 다른 강력한 폐기 패턴은 **블랙리스트(또는 거부 목록) 패턴**입니다. 전통적인 JWT는 무상태이지만, 명시적으로 폐기된(예: 사용자가 로그아웃할 때) "jti" (JWT ID) 클레임의 분산 캐시(예: Redis)를 유지할 수 있습니다. `validate()` 메서드에서 현재 토큰의 ID가 블랙리스트에 존재하는지 확인합니다. 이를 통해 매 요청마다 기본 데이터베이스를 쿼리할 필요 없이 특정 토큰을 거의 즉시 폐기할 수 있으며, 필요한 보안 제어를 추가하면서도 높은 수준의 성능을 유지할 수 있습니다.

고도로 보안이 중요한 환경에서는 발급된 모든 토큰 ID가 "유효한 토큰" 저장소에 있어야 하는 **화이트리스트 패턴**을 구현할 수도 있습니다. 이 방식은 시스템을 사실상 상태 유지(stateful) 방식으로 만들지만, 활성 세션에 대해 가능한 최대의 제어 권한을 제공합니다. Fluo에서는 `validate()`가 비동기적이고 의존성 주입을 지원하기 때문에, 적절한 서비스(예: `RedisService`)를 주입하고 몇 줄의 로직을 추가하는 것만으로 이러한 패턴들을 쉽게 구현할 수 있습니다.

## 15.4 Using AuthGuards
전략이 정의되면 내장된 `AuthGuard`를 사용하여 라우트를 보호할 수 있습니다. `AuthGuard`는 특정 전략 이름(보통 'jwt')에 대한 가드를 생성하는 팩토리입니다. 이는 전략의 검증 로직과 Fluo의 요청 생명주기 사이를 이어주는 접착제 역할을 합니다.

### 15.4.1 Basic Implementation
라우트를 보호하려면 단순히 `@UseGuards()` 데코레이터를 적용하면 됩니다.
...
export class ProfileController {
  ...
  getProfile(@Request() req) {
    // req.user에는 JwtStrategy.validate()가 반환한 Principal이 들어있습니다.
    return req.user;
  }
}
```
...
`@UseGuards()`를 클래스 전체(컨트롤러)에 적용하거나 개별 메서드에 적용할 수 있습니다. 컨트롤러 수준에서 적용하는 것은 "기본적으로 보안 적용(secure by default)" 접근 방식으로, 해당 클래스의 모든 라우트가 보호되도록 보장합니다. 일부 라우트만 공개해야 하는 경우, 더 구체적인 설정이나 커스텀 가드를 사용하여 예외를 처리할 수 있습니다.
...
Fluo를 사용하면 단일 라우트나 컨트롤러에 여러 가드를 쌓을 수 있습니다. `@UseGuards()`에 여러 가드를 제공하면 순서대로 실행됩니다. 만약 어떤 가드라도 `false`를 반환하거나 예외를 던지면 요청은 즉시 차단되고 이후의 가드들은 실행되지 않습니다. 이러한 효율성은 성능 면에서 중요한데, 요청이 이미 승인되지 않은 것으로 간주된 후 불필요한 데이터베이스 조회나 암호화 체크를 피할 수 있기 때문입니다.

내장 가드와 커스텀 가드를 혼합하여 사용할 수도 있습니다. 예를 들어, `AuthGuard('jwt')`를 사용하여 신원을 확인한 다음 커스텀 `IpWhitelistGuard`를 사용하여 사내 네트워크로 접근을 제한할 수 있습니다. 가드의 이러한 조합 가능한 특성은 복잡한 보안 파이프라인을 구축하면서도 각 부분을 독립적으로 추론하고 테스트하기 쉽게 만들어 줍니다.

## 15.5 Role-Based Access Control (RBAC)
인증(누구인가?)은 전투의 절반일 뿐입니다. 나머지 절반은 **인가(무엇을 할 수 있는가?)**입니다. 사용자가 라우트에 접근하기 위해 필요한 역할을 가졌는지 확인하는 `RolesGuard`를 만들어 보겠습니다.

### 15.5.1 Setting Metadata with @Roles
먼저 라우트에 필요한 역할을 표시할 방법이 필요합니다. 이를 위해 커스텀 데코레이터를 사용합니다. Fluo에서 데코레이터는 비즈니스 로직을 더럽히지 않고 코드에 동작 메타데이터를 첨부하는 기본 방법입니다.
...
가드는 이 메타데이터를 읽어 사용자의 `Principal`에 있는 역할과 비교합니다. `Reflector` 서비스를 주입받는 점에 주목하십시오. 이는 애플리케이션의 어느 레벨에서든 데코레이터를 통해 첨부된 메타데이터를 가져올 수 있게 해주는 Fluo의 내장 유틸리티입니다.
...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!requiredRoles) {
      return true; // 필요한 역할이 없으면 접근 허용
    }

    const request = context.switchToHttp().getRequest();
    const user = request.user;

    // 사용자가 필요한 역할 중 최소 하나를 가지고 있는지 확인
...
하나의 라우트에 여러 가드를 사용할 수 있습니다. 가드들은 나열된 순서대로 실행됩니다. 보통 `AuthGuard`를 먼저 실행하여 사용자 신원을 확인한 다음, `RolesGuard`를 실행하여 권한을 확인하는 것이 일반적입니다.

가드를 체이닝할 때 **커스텀 전역 가드(Custom Global Guards)**를 활용할 수도 있습니다. 일반적인 패턴은 애플리케이션 전체에 전역 `JwtAuthGuard`를 적용하되, `@Public()` 데코레이터를 사용하여 특정 라우트만 예외적으로 허용하는 것입니다. 이를 통해 애플리케이션이 "기본적으로 보안 적용(Secure by Default)" 상태가 되도록 보장하며, 필요한 곳에만 명시적으로 구멍을 뚫을 수 있습니다. 이 전략은 개발자가 새로운 컨트롤러에 가드를 추가하는 것을 잊어버려 실수로 민감한 엔드포인트를 노출할 위험을 크게 줄여줍니다.

이러한 "기본 보안 적용" 패턴을 구현하려면 `@fluojs/core`에서 제공하는 `APP_GUARD` 토큰을 사용하여 메인 `AppModule`에 가드를 등록합니다. 이렇게 하면 Fluo는 가드를 인스턴스화하고 애플리케이션에 들어오는 모든 요청에 대해 이를 적용합니다. 가드 내부에서는 커스텀 `@Public()` 메타데이터의 존재 여부를 확인하여 인증 체크를 건너뛸지 결정할 수 있습니다. 보안에 대한 이러한 중앙 집중식 접근 방식은 전문적인 수준의 Fluo 애플리케이션의 특징입니다.
...
이 순서는 매우 중요합니다. 만약 `RolesGuard`가 `AuthGuard`보다 먼저 실행된다면, `request.user` 객체가 아직 생성되지 않아 유효한 관리자라도 "접근 금지" 에러를 받게 될 것입니다. Fluo의 순차적 실행 모델은 각 가드가 이전 가드의 부수 효과(요청 객체 생성 등)에 의존할 수 있도록 보장합니다. 이러한 예측 가능한 흐름은 프레임워크 신뢰성의 핵심입니다.

### 15.5.4 Custom Decorators and @CurrentUser
코드를 더욱 깔끔하게 만들기 위해, 요청에서 사용자 객체를 추출하는 `@CurrentUser` 데코레이터를 만들 수 있습니다. 이는 반복적인 `req.user` 접근을 피하고 핸들러의 가독성을 높여줍니다. 커스텀 데코레이터를 활용함으로써 "사용자가 어떻게 저장되는지"에 대한 구현 세부 사항을 숨기고 컨트롤러 개발자에게 깔끔한 API를 제공할 수 있습니다.
...
때때로 라우트별로 `AuthGuard`의 동작을 커스터마이징해야 할 때가 있습니다. 이를 위해 `AuthGuard()` 팩토리의 두 번째 인자로 옵션 객체를 전달하거나(상속을 통해 사용할 때), 모듈에서 설정할 수 있습니다. 일반적인 옵션으로는 `session: false`(상태가 없는 API를 위해 세션 지원을 비활성화)와 `property: 'principal'`(사용자가 첨부되는 요청 객체의 속성 이름을 변경) 등이 있습니다.
...
복잡한 애플리케이션에서는 여러 인증 방법을 동시에 지원해야 할 수도 있습니다. 예를 들어, 사용자가 표준 JWT 또는 수명이 긴 API 키를 통해 인증할 수 있는 경우입니다. `AuthGuard`에 전략 이름의 배열을 전달하여 이를 구현할 수 있습니다.

여러 전략을 지원할 때, Fluo는 내부적으로 **전략 선택 로직(Strategy Selection Logic)**을 처리합니다. `['jwt', 'api-key']`와 같은 배열을 제공하면, Passport는 지정된 순서대로 각 전략을 사용하여 요청을 인증하려고 시도합니다. 성공적으로 인증된 첫 번째 전략이 "승리"하고, 해당 principal이 요청 객체에 첨부됩니다. 모든 전략이 실패하면 가드는 `UnauthorizedException`을 던집니다.
...
@Get('sensitive-data')
getData() {
  return { message: '이 데이터는 JWT 또는 API 키 중 하나로 보호됩니다' };
}
```

여러 전략이 제공되면 `AuthGuard`는 이를 순서대로 시도합니다. 어떤 전략이라도 성공하면 요청이 허용됩니다. 이러한 "논리적 OR" 동작은 인증 방식 마이그레이션 중에 하위 호환성을 유지하거나 단일 엔드포인트 집합으로 다양한 클라이언트 유형을 지원하는 데 매우 유용합니다.

## 15.6 Customizing Unauthorized Responses
기본적으로 `AuthGuard`는 표준 `UnauthorizedException`을 던집니다. 하지만 에러 메시지를 커스터마이징하거나 보안 감사를 위해 실패한 시도를 로그로 남기고 싶을 수 있습니다. `AuthGuard`를 확장하여 이를 구현할 수 있습니다.
...
export class MyJwtGuard extends AuthGuard('jwt') {
  handleRequest(err, user, info) {
    if (err || !user) {
      // 디버깅을 위해 info.message를 로그로 남깁니다 (예: "jwt expired")
      throw err || new UnauthorizedException('이 리소스에 접근하려면 로그인이 필요합니다');
    }
    return user;
  }
}
...
Fluo의 가드 시스템은 이러한 정교한 에러 관리를 구현하는 데 필요한 도구들을 제공합니다. 이러한 훅을 사용하여 특정 IP에서 만료된 토큰으로 대량의 요청이 들어오는 것과 같은 의심스러운 패턴을 감지했을 때(예: "재전송(replay)" 공격 시도) 자동화된 알림을 트리거할 수도 있습니다. 내부 모니터링 시스템에 구체적인 실패 원인(예: 서명 불일치 vs 만료)을 기록하는 것은 사고 대응 및 포렌식 분석에 필수적입니다.

`handleRequest` 메서드를 확장함으로써 인증 생명주기에 대한 완전한 제어권을 얻게 됩니다. 여기서 데이터를 ELK나 Datadog과 같은 관측성 스택(observability stack)으로 보내는 커스텀 로깅 로직을 구현하여 모든 보안 이벤트가 기록되도록 할 수 있습니다. 이러한 수준의 가시성은 취미 프로젝트와 프로덕션급 엔터프라이즈 시스템을 구분 짓는 중요한 차이점입니다.
...
ABAC의 가장 일반적인 형태는 소유권 확인(Ownership verification)입니다. Fluo에서는 라우트 파라미터에 접근하고 데이터베이스를 조회하는 가드를 만들어 이를 구현합니다.
...
라우트의 90%에 적용되는 정책(예: 인증 필요)이 있는 경우 이를 전역 가드로 등록하는 것을 고려하십시오. 그런 다음 예외적인 몇몇 라우트에 대해서만 "옵트아웃(Opt-Out)" 데코레이터(`@Public()` 등)를 사용할 수 있습니다. 이러한 "기본 거부(Deny by Default)" 방식은 모든 새 컨트롤러에 수동으로 가드를 적용하는 것보다 훨씬 안전하며, 빠른 개발 주기 동안 실수로 데이터가 노출되는 것을 방지합니다.
...
Passport 전략은 실패에 대한 자세한 정보를 제공하는 `info` 객체를 반환할 수 있습니다. `handleRequest` 메서드에서 이 객체를 조사하여 `401 Unauthorized`를 던질지 아니면 `403 Forbidden`을 던질지 결정해야 합니다. 이러한 세분화된 처리는 프론트엔드 개발자가 사용자에게 "세션이 만료되었습니다. 다시 로그인해 주세요" 또는 "잘못된 자격 증명입니다"와 같이 더 나은 피드백을 제공하는 데 도움이 됩니다.

### 15.12.2 Strategy Debugging Techniques
전략 구현에 어려움을 겪고 있다면 `PassportModule` 설정에서 `debug: true` 옵션을 사용하십시오. 이렇게 하면 Passport 내부 상태의 상세 로그가 활성화되어 검증 프로세스의 어느 부분에서 실패하는지 정확히 확인할 수 있습니다. 이는 문제의 원인이 암호화 서명 깊숙이 숨겨져 있을 수 있는 복잡한 OAuth2 또는 SAML 핸드셰이크를 다룰 때 큰 도움이 됩니다.
