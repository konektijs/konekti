...
가드는 요청이 라우트 핸들러로 진행될 수 있는지 여부를 결정하는 특수 클래스입니다. 가드는 인터셉터 체인이 조합되고 실행되기 전에 먼저 통과해야 하며, 그 뒤에 파이프와 핸들러가 이어집니다. 가드는 애플리케이션의 "경비원"과 같아서 권한이 있는 요청만 통과하도록 보장합니다. 보안 로직을 가드에 중앙 집중화함으로써 컨트롤러를 비즈니스 로직에 집중시키고 API 전체에 걸쳐 일관된 보안 태세를 유지할 수 있습니다.
...
또 다른 폐기 패턴은 **블랙리스트(또는 거부 목록) 패턴**입니다. 전통적인 JWT는 무상태이지만, 명시적으로 폐기된(예: 사용자가 로그아웃할 때) "jti" (JWT ID) 클레임의 분산 캐시(예: Redis)를 유지할 수 있습니다. `authenticate()` 메서드에서 현재 토큰의 ID가 블랙리스트에 존재하는지 확인합니다. 이를 통해 매 요청마다 기본 데이터베이스를 쿼리할 필요 없이 특정 토큰을 거의 즉시 폐기할 수 있으며, 필요한 보안 제어를 추가하면서도 성능 부담을 낮출 수 있습니다.

보안 요구가 높은 환경에서는 발급된 모든 토큰 ID가 "유효한 토큰" 저장소에 있어야 하는 **화이트리스트 패턴**을 구현할 수도 있습니다. 이 방식은 시스템을 사실상 상태 유지(stateful) 방식으로 만들지만, 활성 세션에 대한 제어력을 높입니다. Fluo에서는 strategy가 비동기적이고 의존성 주입을 지원하기 때문에, 적절한 서비스(예: `RedisService`)를 주입하고 필요한 검증 로직을 추가해 이러한 패턴을 구현할 수 있습니다.
...
이러한 다중 전략 접근 방식은 특히 **시스템 간 통신(System-to-System Communication)**에 유용합니다. 웹 및 모바일 사용자는 JWT를 사용할 수 있지만, 내부 서비스나 제3자 웹훅은 특수한 API 키나 상호 TLS(mTLS)를 통해 인증을 제공할 수 있습니다. 다만 현재 `@fluojs/passport` 패키지의 `AuthGuard`는 `@UseAuth(...)` 또는 모듈의 `defaultStrategy`에서 결정된 **하나의 활성 전략 이름만** 해석합니다. 즉, 하나의 라우트에서 여러 전략 이름을 자동으로 순서대로 시도하는 동작은 기본 제공되지 않습니다. 그래도 성공한 전략이 모두 같은 principal 형태를 반환하게 만들면 컨트롤러 로직은 단순하게 유지할 수 있지만, 어떤 전략을 선택할지는 명시적으로 관리해야 합니다.

인증 방식 마이그레이션 중이거나 하나의 엔드포인트에서 여러 클라이언트 유형을 지원하기 위해 논리적 OR 동작이 필요하다면, 이를 애플리케이션 수준의 조합 패턴으로 구현하세요. 예를 들어 여러 자격 증명을 순서대로 검사한 뒤 하나의 principal 계약을 반환하는 커스텀 전략을 만들거나, 서로 다른 클라이언트를 별도의 엔드포인트/guard로 분리하되 동일한 downstream 인가 정책을 공유하도록 설계할 수 있습니다.
...
Fluo에서 예외 필터는 `@Catch()` 데코레이터를 사용하여 정의되며 전역, 컨트롤러 수준 또는 메서드 수준에서 적용될 수 있습니다. 이러한 계층 구조는 가드 실행 순서를 그대로 반영하며, 전체 요청-응답 생명주기를 처리하기 위한 일관된 멘탈 모델을 제공합니다. 가드와 필터를 함께 사용하면 인증 판단과 응답 형식을 분리하면서 안전하고 유지보수 가능한 API를 구축할 수 있습니다.
...
정책 서비스는 외부 정책 관리 지점(PAP)이나 Casbin, Oso와 같은 분산 권한 엔진과 통합될 수 있습니다. Fluo의 유연한 공급자(Provider) 시스템을 사용하면 이러한 외부 엔진을 가드가 사용할 수 있는 깔끔한 TypeScript 우선 인터페이스로 쉽게 감싸서 사용할 수 있습니다. 이러한 "서비스로서의 인가(Authorization as a Service)" 모델은 수십 개의 서로 다른 서비스 간에 권한 일관성이 유지되어야 하는 마이크로서비스 아키텍처에서 검증된 방식입니다.
...
Fluo에서는 종종 인터셉터나 스코프 지정 공급자(Scoped provider)를 사용하여 데이터베이스 쿼리 컨텍스트에 `tenant_id`를 직접 주입합니다. 이를 통해 애플리케이션 로직에 버그가 있더라도 "테넌트 A"의 사용자가 "테넌트 B"의 데이터를 실수로 볼 가능성을 낮춥니다. 이러한 "강력한 격리(Hard Isolation)" 전략은 많은 엔터프라이즈 컴플라이언스 프레임워크(SOC2 또는 HIPAA 등)의 핵심 요구 사항입니다. 또한 개발자가 모든 쿼리에 `WHERE tenant_id = ?`를 추가하는 것을 잊는 위험을 줄여 개발 경험도 단순해집니다.
...
인증 전략이 실패한다고 해서 항상 보안 침해를 의미하는 것은 아닙니다. 만료된 토큰이거나, 잘못된 헤더 형식이거나, 설정 불일치일 수도 있습니다. 따라서 실패를 모두 같은 메시지로 뭉개기보다, 내부적으로는 원인을 구분하고 외부 응답은 일관되게 유지하는 흐름이 필요합니다.
