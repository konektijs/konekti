...
`ConfigService.getOrThrow()` 메서드는 이러한 "침묵하는 실패(silent failure)"를 방지하도록 설계되었습니다. 요청한 키가 누락된 경우, fluo는 시작 중에 `InternalServerError`를 발생시켜 배포를 중단합니다. 덕분에 잘못된 설정으로 시스템이 운영되는 상황을 초기에 발견할 수 있습니다.
