...
이 스타터 경로는 기본 Node.js + Fastify 온보딩 경로이며, 런타임 전체 이야기를 대표하지는 않습니다. 공식 런타임/패키지 매트릭스는 `docs/reference/package-surface.ko.md`를 기준으로 보고, 런타임별 시작 세부사항은 출판된 `@konekti/platform-*` 어댑터 README가 계속 소유합니다.
