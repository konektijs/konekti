...
7. 패키지 릴리스 준비/배포는 repo-local `package-publish`가 담당하며, `.changeset/*.md`와 Changesets가 생성한 package-level changelog를 release metadata source of truth로 본다.
...
- 릴리스 준비 handoff에는 target package, target version, dist-tag, release_prerelease, pending changeset 또는 Version Packages PR 상태를 포함한다.
- local publish 또는 legacy release-intent 기반 흐름으로 canonical Changesets release workflow를 우회하지 않는다.
