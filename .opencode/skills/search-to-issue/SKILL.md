...
8. 패키지 변경이 버전업/릴리스 준비로 이어질 수 있으면 `docs/contracts/release-governance.md`, `.changeset/*.md`, package-level `CHANGELOG.md`를 release metadata 기준으로 삼는다.
9. 이 저장소의 승인된 릴리스 메타데이터 경로는 Changesets이다. `tooling/release/intents/*.json`는 legacy reference로만 본다.
...
- release / versioning / publish metadata: `docs/contracts/release-governance.md`, `.changeset/*.md`, `.changeset/config.json`, package-level `CHANGELOG.md`, `.github/workflows/release.yml`
...
공개 `@fluojs/*` 패키지의 release-impact 가능성이 있으면 issue draft에 changeset 필요성(`required` / `needs-check` / `not-required`)도 명시한다. 사용자 영향이 있는 public package 변경은 기능 PR에서 affected package와 semver bump가 담긴 `.changeset/*.md`가 필요하다.
